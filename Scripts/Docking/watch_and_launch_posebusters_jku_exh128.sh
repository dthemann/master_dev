...
DOCKLOG="$S/orai_jku_exh128_run.log"
PBLOG="$S/orai_jku_exh128_posebusters.log"
WLOG="$S/jku_chain.log"
STATE="$S/jku_chain.state"
CFG="$ROOT/Scripts/Docking/Posebusters/posebusters_orai_jku_mgltools_exh128_config.yaml"
OPT="$ROOT/Dockings/Orai_JKU_MGLTools_exh128/mgl_tools/optimization_log.csv"
...
while pgrep -f 'python.*run_orai_mgltools_arm\.py.*orai_jku' >/dev/null 2>&1; do sleep 30; done
say "driver exited"
...
bad=[]
if conv.get("mk_export"): bad.append(f"{conv['mk_export']} mk_export rows — MEEKO poses optimised")
