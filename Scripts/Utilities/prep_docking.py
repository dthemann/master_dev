...
def _build_output_path(
    ...
    *,
    ...
    # Strip the combined suffix from the stem if already present (prevents
    # double postfixes like "_meeko_meeko" or "_mgl_tools_mgl_tools").
...
# ── OpenMM energy-minimisation fallback ──────────────────────────────────────

# Self-contained Python script executed in a conda env that has OpenMM + PDBFixer.
# Accepts three positional args: input_pdb  output_pdb  max_iterations
_MINIMIZE_SCRIPT = '''\
import sys
...
# Remove terminal missing residues — only keep internal gaps.
# MD snapshots often start/end mid-sequence; PDBFixer would try to add
# the full N/C-terminal extensions which breaks template matching.
...
with open(output_pdb, "w") as f:
    app.PDBFile.writeFile(simulation.topology, state.getPositions(), f,
                          keepIds=True)
print("minimization_ok")
'''

# Ordered list of conda env names to try when OpenMM is not in the current env.
_OPENMM_CONDA_ENVS = ["vina", "vina2", "openmm"]
...
def _minimize_pdb(
    ...
    *,
    ...
    """Energy-minimise a PDB to fix distorted bond geometry.

    Tries OpenMM in the current interpreter first.  If OpenMM is not installed
    here, it shells out via ``conda run -n <env>`` into an environment that has
    it (auto-detected or explicitly set with *conda_env*).

    Returns True on success, False on failure (never raises).
    """
    log = _printer(verbose)
    input_str = str(input_pdb)
    output_str = str(output_pdb)

    # --- attempt 1: direct import -------------------------------------------
    try:
        import openmm  # noqa: F401
        from pdbfixer import PDBFixer  # noqa: F401
        log(f"  Running OpenMM minimisation in-process for {input_pdb.name} …")
        exec(compile(_MINIMIZE_SCRIPT, "<minimize>", "exec"),
             {"__builtins__": __builtins__,
              "sys": type("_sys", (), {"argv": ["", input_str, output_str, str(max_iterations)]})})
        if output_pdb.exists():
            log(f"  ✓ Minimised PDB written to {output_pdb.name}")
            return True
    except ImportError:
        pass  # fall through to subprocess
    except Exception as exc:
        log(f"  ⚠ In-process minimisation failed: {exc}")

    # --- attempt 2: subprocess via conda env --------------------------------
    import shutil
    import tempfile

    conda_bin = shutil.which("conda")
    if not conda_bin:
        log("  ⚠ conda not found — cannot run OpenMM minimisation as subprocess")
        return False

    envs_to_try = [conda_env] if conda_env else _OPENMM_CONDA_ENVS

    script_file = Path(tempfile.mktemp(suffix="_minimize.py"))
    try:
        script_file.write_text(_MINIMIZE_SCRIPT)
        for env_name in envs_to_try:
            cmd = [
                conda_bin, "run", "-n", env_name,
                "python", str(script_file), input_str, output_str, str(max_iterations),
            ]
            log(f"  Trying OpenMM minimisation via conda env '{env_name}' …")
            try:
                ...
                log(f"  ⚠ Minimisation timed out in env '{env_name}'")
                continue
            except Exception as exc:
                log(f"  ⚠ Minimisation failed in env '{env_name}': {exc}")
                continue
    finally:
        script_file.unlink(missing_ok=True)

    log("  ✗ OpenMM minimisation not available in any known conda environment")
    return False
...
def convert_protein_with_meeko(
    ...
    *,
    ...
    # Strip the combined suffix from the stem if already present (prevents
    # double postfixes like "_meeko_meeko" or "_mgl_tools_mgl_tools").
    combined_suffix = f"{suffix}{postfix}{custom_postfix}"
    if combined_suffix and stem.endswith(combined_suffix):
        stem = stem[: -len(combined_suffix)]
...
def convert_protein_with_mgltools(
    ...
    *,
    ...
    def _run_mgl_cmd(mgl_cmd, pdb_input):
        """Run MGL Tools prepare_receptor and return (success, error_message)."""
        # Rebuild command with the given PDB input
        rebuilt = list(mgl_cmd)
        for idx, tok in enumerate(rebuilt):
            if tok == "-r" and idx + 1 < len(rebuilt):
                rebuilt[idx + 1] = str(pdb_input)
                break
        r = subprocess.run(rebuilt, capture_output=True, text=True)
        if r.returncode == 0:
    ...
    ok, err_text = _run_mgl_cmd(cmd, input_path.as_posix())

    # ── Fallback: energy-minimise and retry if hydrogen addition crashed ──────
    if not ok:
        is_addh_crash = "addh.py" in err_text or "IndexError" in err_text
        if is_addh_crash:
            log(f"⚠ MGL Tools hydrogen addition failed for {input_path.name} "
                "(likely distorted bond geometry from MD). Attempting OpenMM minimisation …")
            minimised_pdb = input_path.with_name(input_path.stem + "_minimised.pdb")
            if _minimize_pdb(input_path, minimised_pdb, verbose=verbose):
                ok2, err_text2 = _run_mgl_cmd(cmd, minimised_pdb.as_posix())
                if ok2:
                    log(f"✓ MGL Tools succeeded after OpenMM minimisation for {input_path.name}")
                else:
                    message = (f"MGL Tools receptor preparation failed for {protein_pdb} "
                               f"even after OpenMM minimisation: {err_text2[:300]}")
                    raise RuntimeError(message)
            else:
                message = (f"MGL Tools receptor preparation failed for {protein_pdb}: "
                           f"hydrogen addition crashed and OpenMM minimisation is not available. "
                           f"Original error: {err_text[:300]}")
                raise RuntimeError(message)
        else:
            message = f"MGL Tools receptor preparation failed for {protein_pdb}: {err_text[:300]}"
            raise RuntimeError(message)
...
# Global variable for MGLTools path - auto-detected or set by users
def _find_mgltools() -> Optional[Path]:
    """Search common locations for an ADFRsuite / MGLTools installation."""
    candidates = [
        Path.home() / "ADFRsuite-1.0",
        Path("/opt/ADFRsuite-1.0"),
        Path("/usr/local/ADFRsuite-1.0"),
        Path.home() / "mgltools",
        Path("/opt/mgltools"),
    ]
    for p in candidates:
...
MGLTOOLS_PATH: Optional[Path] = _find_mgltools()
