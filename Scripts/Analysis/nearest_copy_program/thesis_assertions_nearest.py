# PROGRAM COPY of Scripts/Analysis/thesis_assertions.py (nearest-copy endpoint). Only SPEC and the
...
SPEC = Path(__file__).resolve().parent / "thesis_expected_values_nearest.yaml"  # program copy
...
def _find_by_md5(digest: str | None, skip: tuple[str, ...] = ()) -> str | None:
    ...
    target_size = next(
        (c.stat().st_size for c in (ROOT / "thesis_latex_nearest/media/media").glob("*.png")
