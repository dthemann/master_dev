...
TPL = "template.pptx"
OUT = "Mann_MasterThesis_Defence_v1.pptx"
...
bullets(tf, [
    "Near-native, RMSD ≤ 2 Å from the crystal ligand, placement and shape",
...
# =============================================== 6  RQ1 ACCURACY VS DEPTH
s, ph = new_slide(L_ONE, "Accuracy Depends on How Deep You Look")
drop(ph[14])
picture(s, f"{MEDIA}/image3.png", 0.20, 0.64, 5.10, 4.32)
tf = textbox(s, 5.55, 0.72, 4.25, 4.20)
col_header(tf, "PB-valid and within 2 Å, n = 303")
bullets(tf, [
    "Rank-1: AutoDock 36.6%, DiffDock 34.0%, EquiBind 18.2%",
    "Best-of-top-15: 65.3%, 55.1% and 26.4%",
    "The rank-1 gap between the two leaders is not resolved, "
    "95% interval −4.2 to +9.4 points",
    "AutoDock separates from top-5 onwards, +13.2 points, p = 7.6e-4",
    "Depth buys 87 extra complexes for AutoDock and 64 for DiffDock",
], size=11, gap=7)
para(tf, "The top-k lift is recall, not precision. Realising it needs a selector "
         "the pipeline does not have.",
     size=9.5, italic=True, colour=GREY, bullet=False, space_before=9)
para(tf, "Fairness caveat: AutoDock was tuned over five search budgets, the two "
         "learned arms only over a choice of refiner.",
     size=9.5, italic=True, colour=GREY, bullet=False, space_before=6)
notes(s, """
Core RQ1 slide. Two messages: the leaders are not separable on the pose they
actually nominate, and everyone has a large reservoir of good poses that ranking
fails to surface.
Anticipated challenge, and it is a fair one: AutoDock was tuned. It ran a ladder
of five exhaustiveness values and the top rung was carried forward, while the two
learned arms were only given a choice of refiner. So a tuned pipeline is being set
against two untuned ones, and the AutoDock rank-1 figure carries a selection
optimism the others do not. The thesis quantifies this: the winner's-curse
correction is between 0.4 and 0.8 percentage points per family, and the largest
selection margin is 3.0 points.
Anticipated challenge: no held-out split. Correct. Variant selection and
evaluation use the same 303 complexes. That is stated in the limitations.
Anticipated challenge: is the non-significant rank-1 result equivalence? No. The
study is underpowered there, the minimum detectable difference is 9.8 points at
80% power, and no equivalence margin was prespecified.
""")

# ================================================= 7  FORM VERSUS PLACEMENT
s, ph = new_slide(L_ONE, "Right Shape, Wrong Place")
drop(ph[14])
picture(s, f"{MEDIA}/image4.png", 0.20, 0.64, 5.10, 4.32)
tf = textbox(s, 5.55, 0.72, 4.25, 4.20)
col_header(tf, "Form only, best-fit RMSD ≤ 1 Å")
bullets(tf, [
    "Rank-1 form: AutoDock 52.5%, DiffDock 52.1%, EquiBind 33.3%",
    "On form the two leaders are indistinguishable at every depth, p = 1.000",
    "So the placement gap on the previous slide is localisation, not "
    "conformer quality",
    "DiffDock builds the right shape and seats it less well",
    "EquiBind is weak on both axes, its rank-1 median already exceeds 2 Å",
], size=11, gap=7)
para(tf, "About six in ten rank-1 failures of the two leaders contain no "
         "qualifying pose at any depth. Generation, not ranking, is the binding "
         "constraint.",
     size=9.5, italic=True, colour=GREY, bullet=False, space_before=10)
notes(s, """
Why the decomposition matters: in-place RMSD mixes two different failures. Kabsch
superposition removes the translation and rotation, so what is left is purely the
internal conformation. A tool can be perfect on form and useless in practice.
Anticipated challenge: is the decomposition exact? No, and the thesis flags it.
The in-place and best-fit values each minimise over symmetry-equivalent atom
mappings independently and can pick different mappings for the same pose, so
placement is reported as a residual diagnostic rather than a measured component.
...
threshold, and the appendix resolves form over ten thresholds so the sensitivity
