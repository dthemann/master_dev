...
\chapter{Source Code Availability}\label{source-code-and-data-availability}}
...
\noindent The repository contains the docking drivers for the three tools (AutoDock Vina, DiffDock and EquiBind), the post-hoc optimisation (smina / gnina) and PoseBusters validity pipeline.
...
\caption[Physics-based docking flow chart]{Physics-based docking flow chart \cite{ref015}, \cite{ref017}, \cite{ref018}, \cite{ref019}, \cite{ref020} and \cite{ref021}.}\label{fig:literature-docking-workflow}
\end{figure}
...
The three end-to-end pipelines share the same downstream validity definitions but differ in receptor preparation and conformer generation. AutoDock Vina and DiffDock receive the same RDKit \cite{ref086} ETKDGv3 + UFF start conformer, whereas EquiBind generates thirty ETKDGv3 + MMFF conformers. In the primary benchmark arms AutoDock Vina, DiffDock and unguided EquiBind all search the whole receptor without a crystal-derived restriction. The separately reported EquiBind variants receive an fpocket- or P2Rank-predicted pocket and crop locally around it, and are set out in Section~\ref{equibind-2}. Tool-specific settings and pose budgets are summarised in Table~\ref{tab:appendix-protocols}, and the corresponding settings for the two Orai1 panels in Table~\ref{tab:appendix-orai-protocols}.

Five preparation choices are recorded here because they cannot be recovered from the output files. Every ligand entered preparation in the tautomer and the ionisation state its source file carried, and no protomer or tautomer enumeration and no pK\textsubscript{a} model ran on any ligand at any stage. The AutoDock ligand path does not preserve that hydrogen set. Each start conformer is converted to MOL2 with Open Babel and passed to \texttt{prepare\_ligand -A hydrogens}, whose valence-based repair builds hydrogens the source file does not carry. The repair fires on 161 of the 308 ligands and adds 353 polar hydrogens. All but one sit on an acceptor nitrogen, which is retyped from \texttt{NA} to \texttt{N} and turned into a donor in the hydrogen-bond term. The prepared set carries 34 \texttt{NA} atoms where the same preparation without the flag carries 386, and 280 of the 308 ligands enter the search with none. The heavy-atom conformer is identical to the source file in all 308 cases, and the hydrogen-bond typing is not. The ADFRsuite preparation described below assigns Gasteiger partial charges to both the ligand and the AutoDock receptor, and both sets are written into the PDBQT files. Crystallographic water is absent from the distribution itself, so every arm docks a water-free receptor. The benchmark ships one coordinate file per entry carrying a crystallographic cell record and no assembly annotation, and that file was docked as supplied, so no biological assembly was generated and no symmetry mate was added.

EquiBind shares none of that receptor path. It passes each receptor through \texttt{reduce} 4.15.250408, which adds hydrogens and optimises rotatable hydroxyl and ammonium orientations without its amide and histidine flip search. Nothing is stripped and no partial charge is assigned, so the EquiBind receptors keep the deposited heteroatom records of 274 of the 308 entries and reach the validity screen with all-atom hydrogens.

All software versions were verified by invoking each binary. Docking used AutoDock Vina v1.2.7-20-g93cdc3d-mod, DiffDock-L (PyTorch 2.4.0 / CUDA 12.4) and a custom EquiBind pipeline (February 2025, PyTorch 2.4.1 / CUDA 12.4). Local optimisation used gnina 1.3.2 and smina (9 November 2017 build, on the Vina 1.1.2 scoring base). PDBQT preparation used ADFRsuite 1.0 \cite{ref089}, whose \texttt{prepare\_ligand} and \texttt{prepare\_receptor} wrap its own bundled copies of the AutoDockTools \cite{ref049} \texttt{prepare\_ligand4.py} and \texttt{prepare\_receptor4.py} utilities, for both ligands and receptors of every AutoDock arm reported here, with receptor cleaning by PDBFixer 1.12.0 on OpenMM 8.3.1. Blind pocket detection for EquiBind used fpocket 4.0 \cite{ref060} and P2Rank 2.5 \cite{ref061}. Physical-validity and interaction screening used PoseBusters 0.6.3 \cite{ref035} and PandaMap 4.1.0 \cite{ref090}. All screening, statistics and figure generation ran in one conda environment holding Python 3.12.12, RDKit 2025.09.1, NumPy 2.3.4, SciPy 1.16.2, pandas 2.3.3 and scikit-learn 1.8.0.

AutoDock Vina was rebuilt from commit 93cdc3d of the upstream development branch with a local purely additive patch introducing the boron with a van der Waals radius of 1.92\angstrom{} and a well depth of 0.29773 kcal/mol. None of the benchmark ligands carries boron. The one boron-bearing compound in this work is 2-APB, and its preparation assigns the new type to the boron centre. Every docked 2-APB file of the reported Orai1 arm records the atom types A, B, C, HD, N and OA, with one boron atom in each pose. The boron of 2-APB was therefore searched under the added type, which supplies an X-Score radius of 1.87\angstrom{} under both Vina and Vinardo, and the Lewis acidity of the boron centre is unmodelled, as it would be under any empirical scoring function of this family.

\hypertarget{autodock-vina-2}{%
\section{AutoDock Vina}\label{autodock-vina-2}}

AutoDock Vina \cite{ref022}, \cite{ref023} provides the classical reference. Receptors are cleaned with PDBFixer, which strips waters, ions, metals and cofactors and maps covalently modified residues onto their standard parents. The modifying atoms that this mapping drops are restored afterwards as uncharged heteroatom records. The ADFRsuite preparation named above then adds hydrogens and Gasteiger partial charges and writes the PDBQT. No benchmark receptor was titrated at a nominal pH. A fallback to that placement exists for receptors the converter cannot hydrogenate, and it fired on the Orai1 Fr0 receptor alone (Appendix~\ref{orai1-receptor-model}). Without a titration model the converter places a hydrogen on both ring nitrogens of every histidine, and all 4,850 histidines of the 308 receptors carry \texttt{HD1} and \texttt{HE2}. 293 of the 308 receptor files therefore contain no \texttt{NA} atom. Receptor nitrogen enters the search as a donor almost everywhere, and oxygen supplies 97.5\% of the receptor acceptor atoms that remain. Ligands use the shared start conformer. The search box spans the whole receptor, so AutoDock searches blindly exactly as DiffDock does. Vina runs at energy range 6 kcal/mol and random seed 42, retaining up to thirty ranked modes per complex. Exhaustiveness is the one setting that was varied, and the reported arm is exhaustiveness 128.

A quarter of the benchmark carries a metal where the ligand binds, and this preparation removes it for the AutoDock and the DiffDock arm. Section~\ref{metal-adjacent-stratum} reports the size of that stratum and what it costs each arm.

The search box is constructed per receptor rather than set to a fixed size. A 1\angstrom{} padding is applied, and every deposited copy of the ligand lies inside the box. Over the 303 analysed receptors the resulting edges run from 29.5 to 167.4\angstrom{} with a median of 63.5\angstrom{}, and the volumes from 44,430 to 2,479,703\angstrom{}\textsuperscript{3} with a median of 263,874\angstrom{}\textsuperscript{3}, a 56-fold range across the cohort. The AutoDock numbers reported here remain blind-docking numbers rather than re-docking numbers.

Eight AutoDock variants were run on this set, five independent docking passes at exhaustiveness 18, 32, 64, 92 and 128 under the default Vina function \cite{ref023}, plus a gnina rescoring pass over three of them. They are tabulated in Table~\ref{tab:appendix-exhaustiveness-ladder}, whose section sets out the basis for the choice.

Three protocol details of the rescoring pass matter for interpretation. The gnina-side empirical objective is the Vina function, matching the function the poses were searched under. Ranking is taken from CNNaffinity rather than gnina\textquotesingle s default CNNscore ordering, with the original Vina rank retained alongside it. The convolutional model was pinned to the crossdock\_default2018 ensemble rather than left at the gnina 1.3 default.
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1350}}@{}}
...
Complexes of 303 with at least one pose that is PoseBusters-valid and within 2\angstrom{} of the nearest deposited copy of the ligand among the first d ranks. The significance column is an exact McNemar test of that arm against the selected arm at top-15, Holm-corrected across the family of seven.} \\
\end{longtable}

The ladder buys progressively less, as Figures~\ref{fig:appendix-exh-cost} and \ref{fig:appendix-exh-return} show against measured search time. Read as complexes recovered per additional hour, on the conjunction of validity, near-nativeness and form over the whole pool, the three measurable steps return +71.6, +35.9 and +5.9 complexes per hour. The contrast between the first and last, recomputed on the same bootstrap-resampled complexes, is +65.6 per hour with a 95\% interval of +32.1 to +137.2. That interval lies wholly above zero, which is the diminishing-returns claim and the single cell fixed in advance for a confirmatory reading. In absolute terms the five rungs recover 98, 128, 149, 159 and 164 complexes. The first step adds thirty and the last adds five. Those counts and the confirmatory interval above are kept on the single deposited instance on which the cell was fixed in advance. Scored against the nearest deposited copy the same rungs recover 115, 143, 166, 172 and 178 complexes. The first step adds twenty-eight and the last six. The three measurable steps return +66.8, +39.3 and +3.5 complexes per hour, and the first-to-last contrast is +62.4 per hour with a 95\% interval of +32.0 to +133.3.
...
Analysis ranking & gnina CNNaffinity in the rescored arms, Vina score in the raw arms & Model confidence & minimised Vina energy, most negative first \\
...
Cost is charged as device occupancy throughout, on the single basis the Methods define, namely CPU-core-seconds divided by the 32 hardware threads of the workstation plus GPU-seconds. The search consumes 151.42 CPU-core-hours, the 4.73-hour search wall multiplied by the 32 requested threads rather than a measured occupancy, and dividing it by 32 therefore returns the time the search actually took. The rescoring pass ran sixteen ways concurrently on one GPU. Its GPU-seconds are the union of the per-pose optimiser intervals recorded in the provenance sidecars, 0.71 hours over the 303 complexes. The complexes ran one after another, and the per-complex unions therefore sum exactly to that campaign figure. Summing the sixteen workers\textquotesingle{} elapsed times instead would give 10.27 GPU-hours, more than one device can deliver in 0.71 hours, and is not used. Each worker also held about 2.1 processor cores, the value recorded in the run configuration, and the pass therefore adds 21.6 CPU-core-hours, or 0.67 charged hours, to the 4.73 of the search. AutoDock totals 6.12 charged hours with 22.6\% for rescoring, and the GPU occupancy alone is 11.6\% of that total. The parts are rounded separately. The DiffDock GPU stage ran essentially one at a time, and its GPU column already equals the time its device was busy. Its docking process also held host cores, which were not measured and are not charged, so processor time is charged only where a measurement exists. The EquiBind inference seconds are summed over its own worker pool, which overstates that arm by under a fifth of an hour and is left unchanged.
...
\bottomrule\noalign{}
\endlastfoot
AutoDock Vina + gnina & 2.45 & 0.29 & 69.38 \\
...
Entries are pooled over the whole campaign as a ratio of totals rather than as the per-complex medians quoted in the Results, and the denominator is every pose the pipeline generated rather than only the qualifying ones. The charged column follows the Results convention. For AutoDock it is 4.73 search hours, 0.67 hours of rescoring processor time and 0.71 hours of measured GPU occupancy.} \\
\end{longtable}
...
AutoDock Vina is the one arm where that counterfactual can be computed, because its thirty modes are a single deterministic ranked list whose first ten are exactly what a ten-mode run writes. Restricting the experimental panel to those ten, which matches the control panel\textquotesingle{}s selection rule, raises its usable yield from 54.2 to 65.8\% pooled and its per-unit median from 60.0 to 75.0\%, while leaving the control figures untouched. The benchmark-to-experimental gap then falls from 28.7 to 17.0 percentage points, and the contrast no longer clears Holm correction at either inferential unit, moving from an adjusted 0.0006 to 0.054 at the frame-ligand unit and from 0.012 to about 0.2 at the ligand unit, with Cliff\textquotesingle s \ensuremath{\delta} falling from +0.59 to +0.37 and from +0.94 to about +0.6. The ligand-unit value is quoted loosely because three experimental ligands against 308 controls leave it moving between 0.20 and 0.23 under equally defensible handling of the ties. For this arm the selection therefore runs with the reported gap rather than against it, because the gnina ranking orders poses by predicted affinity while the endpoint scores placement, and the two are not aligned on this receptor.

That misalignment is not a property of the three modulators. It reproduces on the control panel, where the same receptor frames are searched with 308 unrelated ligands. Over those 12,315 poses the gnina order places 77.3\% of its first five ranks outside the slab against 90.1\% of ranks six to ten, a gap of 12.8 percentage points, whereas the Vina order moves by 0.5 points between the same bands. Rank correlates with transmembrane occupancy at Spearman \ensuremath{-}0.211 under the gnina order and \ensuremath{-}0.004 under the Vina order, the latter being indistinguishable from independence at p = 0.63 while the former is not at p = 7e-124. The learned ranker therefore promotes membrane-buried poses across chemotypes, which is consistent with an affinity objective rewarding burial on a channel searched without a bilayer or a permeant ion. The bias is measurable on both panels but consequential only on the experimental one, since the control units wrote at most ten modes and retain all of them, so nothing there is discarded by the cap. The reported figures retain the cap on each tool\textquotesingle s own output order, which is what a user of that pipeline would see, but the AutoDock cross-panel contrast should be read as bounded below by the prefix-matched values rather than as the adjusted significance the Results quote.
...
Poses written per unit & Exp.~30 on every unit, Ctrl.~10 on 1,227 of the 1,232 units it covers and nine on the remaining five & Exp.~30 on every unit, Ctrl.~10 requested but returned in full on only 1,037 of 1,215 units & Exp.~30 on every unit, Ctrl.~10 on each of the 1,232 processed units \\
...
Analysis ranking & gnina CNNaffinity order for both the top-ten cap and the top-three interaction cap & Model confidence & minimised Vina energy, most negative first \\
...
\caption[Orai Fr300 with docked ligands and transmembrane exclusion]{Orai Fr300 with docked ligands and transmembrane exclusion.}\label{fig:results-r12}
\end{figure}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1500}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1300}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1550}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1550}}@{}}
...
Entries are per-complex mean counts per pose, which is the inferential unit, whereas Figure~\ref{fig:results-r20} plots pose-level means. AD is AutoDock Vina + gnina and DD is DiffDock + smina. Cliff\textquotesingle s \ensuremath{\delta} is oriented so that a positive value means the benchmark ligands are higher.} \\
\end{longtable}
...
Sanitisation & The molecule passes RDKit\textquotesingle{}s chemical sanitisation checks. \\
...
Internal energy ratio & The UFF energy of the pose is at most 100\ensuremath{\times} the mean energy of a 50-conformer ETKDGv3 ensemble (no gross strain). \\
\multicolumn{2}{@{}l@{}}{%
...
Pocket detection identifies the ion pore as the top-ranked cavity in each frame. In the three thermally sampled frames the cavity is elongated, spanning M1 residues 66 to 110, with additional transmembrane residues in the more open frames. In Fr0 the top-ranked pocket is a compact seven-residue set confined to M1 residues 87 to 109, so the elongated span is a property of the sampled frames rather than of the starting geometry. This region overlaps the functionally implicated pore and filter environment \cite{ref070}, \cite{ref071}, but it is not a conventional buried hydrophobic pocket. The complementary geometric finder returns hundreds of low-druggability cavities, illustrating why blind site selection is unusually difficult on this membrane channel.
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.1472}}
...
\toprule\noalign{}
{\raggedright \textbf{Descriptor}} & {\centering \textbf{Median}} & {\centering \textbf{IQR (25--75\,\%)}} & {\centering \textbf{Range (min--max)}} \\
\midrule\noalign{}
\endfirsthead
...
\toprule\noalign{}
{\raggedright \textbf{Ligand}} & {\centering \textbf{Formula}} & {\centering \textbf{MW (Da)}} & \shortstack[l]{\textbf{Heavy}\\\textbf{atoms}\strut} & \shortstack[l]{\textbf{Rot.}\\\textbf{bonds}\strut} & \shortstack[l]{\textbf{Arom.}\\\textbf{rings}\strut} & {\centering \textbf{HBD / HBA}} & {\centering \textbf{TPSA (\angstrom{}\textsuperscript{2})}} & {\centering \textbf{cLogP}} & \shortstack[l]{\textbf{Ro5}\\\textbf{viol.}\strut} \\
\midrule\noalign{}
\endfirsthead
...
\bottomrule\noalign{}
\endlastfoot
2-APB & C\textsubscript{14}H\textsubscript{16}BNO & 225.1 & 17 & 5 & 2 & 1 / 2 & 35.3 & 0.77 & 0 \\
...
\toprule\noalign{}
{\raggedright \textbf{Variant}} & {\centering \textbf{k}} & {\centering \textbf{Near-native\% {[}95\% CI{]}}} & {\centering \textbf{Validity-aware\% {[}95\% CI{]}}} \\
\midrule\noalign{}
\endfirsthead
...
\toprule\noalign{}
{\raggedright \textbf{Tool}} & {\centering \textbf{k}} & {\centering \textbf{raw\% \ensuremath{\rightarrow} opt\%}} & {\centering \textbf{discordant (raw-only / opt-only)}} & {\centering \textbf{p (raw)}} & {\centering \textbf{p (Holm)}} & {\centering \textbf{sig}} \\
\midrule\noalign{}
\endfirsthead
\caption[]{Within-Tool Optimisation Effect on Cumulative Top-k Near-Native Recovery (continued)}\tabularnewline
\toprule\noalign{}
{\raggedright \textbf{Tool}} & {\centering \textbf{k}} & {\centering \textbf{raw\% \ensuremath{\rightarrow} opt\%}} & {\centering \textbf{discordant (raw-only / opt-only)}} & {\centering \textbf{p (raw)}} & {\centering \textbf{p (Holm)}} & {\centering \textbf{sig}} \\
\midrule\noalign{}
\endhead
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1067}}
  >{\centering\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1067}}
  >{\centering\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1066}}
  >{\centering\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1066}}@{}}
...
\toprule\noalign{}
{\raggedright \textbf{Tool}} & {\raggedright \textbf{Criterion}} & {\centering \textbf{1--10\\smina}} & {\centering \textbf{1--10\\gnina}} & {\centering \textbf{11--20\\smina}} & {\centering \textbf{11--20\\gnina}} & {\centering \textbf{21--30\\smina}} & {\centering \textbf{21--30\\gnina}} \\
\midrule\noalign{}
\endfirsthead
...
Entries are the change in the per-rank rate (percentage points) averaged within each pose-rank band, with every pose held at its own confidence rank for DiffDock or generation rank for EquiBind.} \\
\end{longtable}
\endgroup
...
\toprule\noalign{}
{\raggedright \textbf{Comparison (near-native\%)}} & {\centering \textbf{k = 1}} & {\centering \textbf{k = 5}} & {\centering \textbf{k = 10}} & {\centering \textbf{k = 15}} \\
\midrule\noalign{}
\endfirsthead
...
\bottomrule\noalign{}
\endlastfoot
AutoDock Vina + gnina vs DiffDock (raw) & 49.8 vs 43.6 & 67.3 vs 50.2 & 70.0 vs 52.1 & 71.3 vs 52.8 \\
AutoDock Vina + gnina vs DiffDock (gnina-opt) & 49.8 vs 45.5 & 67.3 vs 54.1 & 70.0 vs 57.1 & 71.3 vs 59.4 \\
DiffDock (gnina-opt) vs EquiBind (gnina-opt) & 45.5 vs 19.5 & 54.1 vs 26.1 & 57.1 vs 28.4 & 59.4 vs 28.4 \\
AutoDock Vina + gnina vs EquiBind (gnina-opt) & 49.8 vs 19.5 & 67.3 vs 26.1 & 70.0 vs 28.4 & 71.3 vs 28.4 \\
\multicolumn{5}{@{}p{\dimexpr\columnwidth-8\tabcolsep\relax}@{}}{\footnotesize%
Entries are complex-level near-native recovery percentages over the N = 303 calibration complexes, computed on the blind whole-protein run, and they match the near-native column of Table~\ref{tab:appendix-top-k-recovery}. These four contrasts are the between-tool Holm family of sixteen described at the head of this chapter, corrected separately from the within-tool family of Table~\ref{tab:appendix-refinement-mcnemar}.} \\
\end{longtable}
\endgroup
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2001}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2001}}@{}}
...
\toprule\noalign{}
{\raggedright \textbf{Variant}} & {\centering \textbf{k = 1}} & {\centering \textbf{k = 5}} & {\centering \textbf{k = 10}} & {\centering \textbf{k = 15}} \\
\midrule\noalign{}
\endfirsthead
...
\midrule\noalign{}
\endfirsthead
\caption[]{Sensitivity of the Validity-Aware Recovery Endpoint to the Reference Convention (continued)}\tabularnewline
\toprule\noalign{}
{\raggedright \textbf{Tool}} & \textbf{d} & \multicolumn{3}{c}{\textbf{Recovery}} & \multicolumn{2}{c}{\textbf{Form}} & \multicolumn{2}{c}{\textbf{Combined}} & \textbf{Single copy} \\
\cmidrule(lr){3-5}\cmidrule(lr){6-7}\cmidrule(lr){8-9}\cmidrule(lr){10-10}
 & & nearest & instance & gain & nearest & instance & nearest & instance & both \\
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0620}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0670}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0650}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0650}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0650}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0910}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0620}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0670}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0650}}
  >{\centering\arraybackslash}p{(\columnwidth - 24\tabcolsep) * \real{0.0650}}
...
Exhaustiveness was raised without ever narrowing the box, so under-sampling and blind search are separated only along the sampling axis. A run against a predicted pocket was not performed, and the comparison with a site-specific protocol therefore remains inferred from the literature rather than established here.
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.1859}}
  >{\centering\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.2139}}@{}}
...
Table~\ref{tab:results-near-native-form} resolves the recovery plotted in Figures~\ref{fig:results-r2a} and \ref{fig:results-r2b} across ten distance thresholds and three ranking depths, for both the as-placed and the best-fit (Kabsch) criterion, each taken to the deposited copy of the ligand nearest to the pose. The 2\angstrom{} in-place column at each depth reproduces the pass-all recovery of Table~\ref{tab:results-depth-recovery}, and the 1\angstrom{} Kabsch column is the form endpoint the Results quote. Paired tests on the in-place criterion were prespecified at 1\angstrom{} and 2\angstrom{} only, Holm-corrected across those two thresholds and the three tool pairs. At rank-1 AutoDock Vina + gnina leads DiffDock + smina by +9.9 points at 1\angstrom{}, with 59 against 29 discordant complexes at p\_holm = 0.0036, and by +5.6 points at 2\angstrom{}, with 73 against 56 discordant complexes at p\_holm = 0.159. At top-15 the 1\angstrom{} contrast is unresolved, with 66 against 47 discordant complexes at p\_holm = 0.090. The remaining eight thresholds carry no paired test and are descriptive.

\begingroup
\scriptsize
\setlength{\tabcolsep}{2pt}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0600}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.2080}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0910}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0080}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0633}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0633}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0633}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0633}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0633}}
...
\multirow{9}{*}{\rotatebox[origin=c]{90}{RMSD}} & AutoDock Vina + gnina & 1 & \multicolumn{2}{r}{%
...
\multirow{9}{*}{\rotatebox[origin=c]{90}{Kabsch RMSD}} & AutoDock Vina + gnina & 1 & \multicolumn{2}{r}{%
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0579}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0602}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.1188}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0553}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0553}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0553}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0569}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0569}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0569}}
  >{\raggedleft\arraybackslash\hspace{0pt}}p{(\columnwidth - 26\tabcolsep) * \real{0.0553}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0634}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0657}}
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.0593}}@{}}
...
AutoDock Vina + gnina (5) & 1,147 & 272 & 89.8\% & 3.291 & 1.615 & 5.926 & 1.215 & 0.645 & 1.962 & 0.178 & 63.7\% & 19.6\% & 16.7\% \\
AutoDock Vina + gnina (15) & 2,832 & 286 & 94.4\% & 4.922 & 2.847 & 7.314 & 1.416 & 0.776 & 2.201 & 0.097 & 77.3\% & 13.9\% & 8.8\% \\
DiffDock + smina (1) & 224 & 224 & 73.9\% & 1.532 & 0.868 & 3.843 & 0.890 & 0.499 & 1.421 & 0.458 & 41.5\% & 29.0\% & 29.5\% \\
DiffDock + smina (5) & 1,041 & 256 & 84.5\% & 1.683 & 0.866 & 4.286 & 0.883 & 0.503 & 1.489 & 0.416 & 44.5\% & 31.2\% & 24.3\% \\
DiffDock + smina (15) & 2,801 & 269 & 88.8\% & 2.062 & 0.945 & 4.935 & 0.936 & 0.520 & 1.603 & 0.377 & 47.1\% & 30.5\% & 22.4\% \\
...
EquiBind + gnina (15) & 1,932 & 169 & 55.8\% & 4.195 & 2.365 & 6.556 & 1.354 & 0.808 & 2.118 & 0.132 & 70.5\% & 18.1\% & 11.4\% \\
\multicolumn{14}{@{}p{\dimexpr\columnwidth-14\tabcolsep\relax}@{}}{\footnotesize%
All rows are the blind whole-protein run with the selected variant of each family, namely AutoDock Vina + gnina, DiffDock + smina and EquiBind + gnina.} \\
\end{longtable}
\endgroup
