...
Cost is charged throughout on the single formula the Methods define, namely CPU-core-seconds divided by the 32 hardware threads of the workstation plus GPU-seconds. Search and rescoring consume 151.42 CPU-core-hours and 10.27 GPU-hours. The CPU value is the search wall multiplied by the 32 requested threads rather than a measured occupancy, and dividing it by 32 therefore returns the 4.73 wall-hours the search actually took. The GPU value is not an occupancy in that sense. The rescoring pass ran sixteen ways concurrently on one device, and the device was busy for 0.71 hours, the union of the per-pose optimiser intervals. The 10.27 figure counts each of the sixteen workers separately and so exceeds what one GPU delivered by about fourteen times. Charging the pass at the measured occupancy instead puts AutoDock Vina at 5.44 total hours with 13.1\% for rescoring, and lowers its charged cost per qualifying pose below that of DiffDock on the reported medians, leaving the two no longer separable on the complexes all three tools solve. This thesis reports the summed figure. The cost ranking of AutoDock against DiffDock is therefore conditional on that choice rather than a property of either method. The DiffDock GPU stage ran essentially one at a time. The EquiBind inference seconds are an undivided sum over their own worker pool as well, although correcting them would move that arm by under a fifth of an hour. The AutoDock column is the only one materially inflated. The processor comparison is unaffected, since it does not involve the GPU term.
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 26\tabcolsep) * \real{0.1280}}
