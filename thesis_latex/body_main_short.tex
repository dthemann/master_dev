...
The Orai1 molecular-dynamics snapshots, Fr0, Fr300, Fr400 and Fr499, represented limited receptor flexibility \cite{ref038}, \cite{ref072} and received no pH-based protonation. The PDBQT converter added polar hydrogens by its geometric rules rather than a titration model (Appendix~\ref{orai1-receptor-model}). The three compounds were docked as neutral species. These are 2abp-NH2 (the neutral protonation state of 2-APB and hereafter called 2-APB), GSK-7975A \cite{ref041}, \cite{ref070} and Synta-66 \cite{ref071}. Because no titration model was applied, these forms are modelling choices rather than predictions of physiological protonation. The externally supplied optimised geometries lacked records of both the optimisation program and its level of theory (Appendix~\ref{experimental-ligand-pharmacology}).
...
A tool can generate a near-native pose without ranking it first. Accuracy is therefore reported for the realistic top-1 pose and for the oracle (best) pose anywhere in the output. Per-rank success and cumulative recovery within the first k poses use the gap between these regimes to distinguish ranking from sampling quality.
...
The calibration analysis included 303 of the 308 source complexes since DiffDock failed to produce poses for 5 complexes (cp. Appendix~\ref{cohort-and-exclusions}). Pipelines searched the whole receptor and were evaluated before and after smina or gnina local optimisation. Variants were chosen based on quality of the produced poses, i.e. being PoseBusters-valid, within 2\angstrom{} in place and within 1\angstrom{} in Kabsch form (cp. Table~\ref{tab:results-pose-production}). Cost and interaction results were excluded from selection and will be evaluted individually. Cost is charged per PoseBusters-valid pose within 2\angstrom{}, and the interaction analysis uses only valid poses from each family\textquotesingle s selected variant.

Table~\ref{tab:results-pose-production} reports up to thirty poses per complex for every variant and thus covers the full pool rather than one selected pose. AutoDock was run over five exhaustiveness values, yet the table prints only the variant carried forward (Appendix~\ref{exhaustiveness-selection}). Rescoring cannot extend a pool, so at that variant raw search leads its own rescored sibling, 164 complexes against 160. The full-pool endpoint cannot express the ranking gain that motivates rescoring. The two DiffDock refiners each recover 138 complexes. They agree on 133 and differ on five in each direction (exact McNemar p = 1.000), although gnina has higher pooled physical validity than smina (87.1\% versus 84.5\%). For unguided EquiBind, gnina also exceeds smina in pooled validity (51.7\% versus 39.2\%) and complex count (42 versus 39), but not significantly in the latter (McNemar p = 0.607).

Since the full-pool endpoint fails to separate leading alternatives, selection uses the top-15 ranking depth\footnote{Poses are ordered within a variant by the ranking that variant makes available. The top-15 depth is used rather than rank-1 because rank-1 rests on a single pose per complex and therefore scores the ranking function as much as the configuration under selection.}. AutoDock Vina at exhaustiveness 128 with gnina rescoring reaches 160 complexes and is considered the leading configuration. Appendix~\ref{exhaustiveness-selection} elaborates on the examined AutoDock configurations. DiffDock with smina reaches 133, compared with 132 for gnina. Unguided EquiBind with gnina reaches 42, compared with 39 for smina. Pocket guidance is worse than no guidance at all for EquiBind, and no fpocket- or P2Rank-guided arm exceeds 18 at any depth.\footnote{Appendix~\ref{equibind-pocket-guided} elaborates on pocket-guided EquiBind arms.}

DiffDock + smina is carried forward on the selection criterion, leading gnina by 133 to 132 complexes and by 834 to 828 qualifying poses at the top-15 ranking depth. Since the arms disagree on nine complexes, including four recovered only by gnina the ordering is unstable. Morever, Table~\ref{tab:results-pose-production} gives gnina higher pooled validity and near-native recovery (176 versus 173 complexes), but smina higher form recovery (237 versus 235), i.e. components also conflict.  Both DiffDock refiners locally minimise each pose within a pose-centred 4\angstrom{} box. While smina energy minimization improves pose quality, DiffDock native ranking outperforms the smina ranking \footnote{It nominates a near-native rank-1 pose for 32.7\% of complexes, compared with 33.7\% under confidence ranking on those same coordinates. The gnina equivalent reaches 32.3\%.}. Consequently, DiffDock maintains it native confidence rank applied to refined coordinates. This is a narrower operation than the gnina step applied to AutoDock, minimizing and re-orderering poses by CNN affinity. Since, EquiBind provides no native confidence score, poses are ranked by gnina affinity.

The selected variants are AutoDock Vina at exhaustiveness 128 with gnina rescoring, DiffDock + smina and unguided EquiBind + gnina scoring. Subsequent references to AutoDock, DiffDock or EquiBind denote these variants. Figure labels use AutoDock*, DiffDock* and EquiBind*.

% UPDATED 2026-08-12: table repopulated from the WHOLE-PROTEIN run (notebook Master_Docking_AD_Full_Protein.ipynb cell 34).
% Source: posebusters_results/benchmark_full_protein_vina_scoring/dock/pose_comparison_report/per_pose_metrics.csv (303 complexes).
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 28\tabcolsep) * \real{0.0583}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 28\tabcolsep) * \real{0.0603}}@{}}
\caption{Pose Production and Physical-Validity Summary of the Reported Docking Variants}\label{tab:results-pose-production}\tabularnewline
\toprule\noalign{}
{} & {} & {} & \multicolumn{3}{c}{%
...
\midrule\noalign{}
\endfirsthead
\caption[]{Pose Production and Physical-Validity Summary of the Reported Docking Variants (continued)}\tabularnewline
\toprule\noalign{}
{} & {} & {} & \multicolumn{3}{c}{%
PoseBusters} & \multicolumn{3}{c}{%
\shortstack[c]{Near-Nativeness\\
...
+ gnina rescoring & & 8,976 & 301 & 8,880 & 98.9\% & 202 & 319 & 3.6\% & 227 & 2,836 & 31.6\% & 160 & 221 & 2.5\% \\
...
EquiBind smina & & 9,090 & 218 & 3,565 & 39.2\% & 55 & 339 & 3.7\% & 184 & 2,521 & 27.7\% & 39 & 157 & 1.7\% \\
EquiBind gnina & & 9,090 & 248 & 4,703 & 51.7\% & 65 & 373 & 4.1\% & 196 & 2,578 & 28.4\% & 42 & 187 & 2.1\% \\
...
\caption[Post-hoc optimisation and PoseBusters validity]{Post-hoc optimisation and PoseBusters validity. Significance brackets carry Holm-corrected p-values, where *** p \textless{} 0.001, ** p \textless{} 0.01 and * p \textless{} 0.05.}\label{fig:results-r1}
\end{figure}

Figure~\ref{fig:results-r1} reports per-complex PoseBusters-validity yields before and after local optimisation. AutoDock's raw search is already almost perfectly valid. The median per-complex yield is 100\%, pooled validity is 99.6\%, and every one of the 303 complexes has at least one valid pose. Only 33 of 8,976 raw poses fail PoseBuster checks, split across internal clash (12), minimum distance to protein (20) and internal energy (1). Gnina rescoring leaves the median at 100\% and moves pooled validity to 98.9\%. The Hodges-Lehmann difference is exactly zero percentage points because 293 of the 303 complexes are unchanged, and the small movement that exists runs against rescoring (Wilcoxon p = 0.081). The two arms are level on physical validity with no headroom for an optimiser to repair.
DiffDock rises from a 13.3\% raw median to 93.3\% with smina, a Hodges-Lehmann gain of +62.1 percentage points. Unguided EquiBind rises from 0\% to 56.7\% with gnina. These near-unanimous gains survive Holm correction, with matched-pairs rank-biserial effects of 0.994--1.000 and adjusted p-values below 10\textsuperscript{-35}. DiffDock behaves similarly with gnina (96.6\%), whereas EquiBind performs worse with smina (26.7\%). The refiners are thus nearly interchangeable for DiffDock, while gnina is decisive for EquiBind.
...
EquiBind & 15.2\% (46) & 20.1\% (61) & 20.5\% (62) & +15 (+5.0\%) *** & +1 (+0.3\%) \\
\multicolumn{6}{@{}p{\dimexpr\columnwidth-6\tabcolsep\relax}@{}}{\footnotesize%
Each family is reported at its selected variant. Two-sided McNemar tests with Holm-Bonferroni correction, *** p \textless{} 0.001, ** p \textless{} 0.01 and * p \textless{} 0.05} \\
\multicolumn{6}{@{}p{\dimexpr\columnwidth-6\tabcolsep\relax}@{}}{\footnotesize%
Recovery is the same PoseBusters-valid, within-2\angstrom{} recovery reported across distance thresholds in the near-nativeness and form analysis (Table~\ref{tab:results-near-native-form}), coinciding at every reported depth (111/198/199 for AutoDock at rank-1/top-15/top-30).} \\
\end{longtable}
...
% benchmark_full_protein_vina_scoring/.../per_pose_metrics.csv, DOMINANT AutoDock arm
...
% DiffDock-smina 103/167/169, EquiBind-gnina 46/61/62.
% Table 6 decomposition, same gnina arm: near-native gained +61/+63/+17; of which remain PB-invalid 0/-1/-2;
% valid rescued +1/+2/+0; net +62/+64/+15. (Raw Vina arm would give +67 near-native and +67 net for AutoDock.)
...
% p = 1.0/0.5/1.0, none significant (no stars). Counts 151->151, 167->169, 61->62.
...
EquiBind remains weakest on form, rising from 30.7\% at rank-1 to 49.8\% at best-of-top-30. This indicates internal-geometry error in addition to misplacement and agrees with evaluations in which direct regression trails diffusion and hybrid methods on combined geometry and validity \cite{ref013}, \cite{ref035}. The 19.1-point depth gain provides evidence of acceptable but mislocated shape poses. From top-15 to top-30, placement gains at most 0.7 points, whereas form at 1\angstrom{} gains 2.3 points for both AutoDock and DiffDock and 3.6 for EquiBind (Table~\ref{tab:results-near-native-form}).
...
Poses from the three selected variants were pooled without validity filtering, clustered and scored for crystal-site recovery under the 4\angstrom{} rule defined in the Methods. Fourteen of 303 complexes have no qualifying cluster and count as non-reaches. Figure~\ref{fig:results-r4} and Table~\ref{tab:results-cluster-recovery} report top-N recovery for individual tools and combinations.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image6.png}
\caption{Top-N crystal-cluster co-recovery. Each lower-triangular panel is one ranking depth, and the starred labels mark the selected variant of each family}\label{fig:results-r4}
\end{figure}

AutoDock and DiffDock have almost fully overlapping reach intervals at every depth, while EquiBind trails both. From rank-1 to top-15, DiffDock gains 27 percentage points, AutoDock 24 and EquiBind 7. Pairings become less redundant with depth but remain positively associated. At top-15, all converge at \ensuremath{\phi} = +0.21 to +0.27 with Fisher p \ensuremath{\leq} 7e-4.
...
\bottomrule\noalign{}
\endlastfoot
AutoDock reach \% {[}95\% CI{]} & 60 {[}54--65{]} & 80 {[}75--84{]} & 83 {[}78--86{]} & 84 {[}80--88{]} \\
DiffDock reach \% {[}95\% CI{]} & 56 {[}50--61{]} & 77 {[}71--81{]} & 80 {[}75--84{]} & 83 {[}78--86{]} \\
EquiBind reach \% {[}95\% CI{]} & 39 {[}34--45{]} & 44 {[}38--49{]} & 46 {[}40--51{]} & 46 {[}41--52{]} \\
AutoDock + DiffDock co-reach\footnote{Co-reach \ensuremath{\phi} is the signed association between two tools\textquotesingle{} per-complex reach against independence, tested two-sided by Fisher, where a positive value means the pair succeeds and fails together and a negative value means it is complementary. All three pairings are estimable in the whole-protein run. All three pairings reach p \textless{} 0.05 on a two-sided Fisher test. Three-way agreement exceeds chance at every depth with a permutation p below 1e-4, and the ranking depths are nested cumulative thresholds rather than an independent test family.} \ensuremath{\phi} & \textbf{+0.33} & \textbf{+0.19} & \textbf{+0.22} & \textbf{+0.21} \\
AutoDock + EquiBind co-reach \ensuremath{\phi} & \textbf{+0.41} & \textbf{+0.24} & \textbf{+0.26} & \textbf{+0.27} \\
DiffDock + EquiBind co-reach \ensuremath{\phi} & \textbf{+0.44} & \textbf{+0.31} & \textbf{+0.26} & \textbf{+0.27} \\
All three reach, complexes observed / expected & 86 / 40 & 111 / 81 & 119 / 91 & 125 / 98 \\
\multicolumn{5}{@{}p{\dimexpr\columnwidth-5\tabcolsep\relax}@{}}{\footnotesize%
The three reach rows are percentages of the 303 complexes, carried with Wilson 95\% intervals in the same units. The final row counts complexes instead and is not a percentage.} \\
\end{longtable}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image7.png}
\caption[Composition and geometry of the crystal-closest cluster]{Composition and geometry of the crystal-closest cluster (n = 303, top-15 poses per tool, the starred labels marking the selected variant of each family). Rates are taken over all 303 complexes, of which 16 contribute no pose to any panel.}\label{fig:results-r5}
\end{figure}

Figure~\ref{fig:results-r5} (A) gives DiffDock a median six of fifteen poses in the crystal cluster and 83\% reach, and AutoDock five poses and 84\% reach. EquiBind has a median of zero and 46\% reach. In the reported top-15 analysis the two leading tools differ in pose count, where DiffDock contributes more (Holm-corrected rank-biserial \ensuremath{-}0.30, p \textless{} 1e-4), but not in reach (McNemar p = 0.539 on 36 against 30 discordant complexes). Both exceed EquiBind on reach. Panel (B) shows polarised consensus, with 125 complexes solved by all tools versus 97.6 expected (permutation p \textless{} 1e-4), and 16 by none versus 4.4 expected. AutoDock accounts for 28 of the 53 single-tool solutions, compared with 24 for DiffDock and one for EquiBind. Panel (C) plots, for each complex and tool, the standard deviation of that tool\textquotesingle s in-cluster poses in their distance to the crystal centroid. The median is 0.67\angstrom{} for AutoDock over 236 defined complexes, 0.57\angstrom{} for EquiBind over 134 anatd 0.36\angstrom{} for DiffDock over 232. Cohorts differ because the statistic requires at least two within-cluster poses, making missingness informative. The measure is radial, and poses lying at a common distance from the crystal centroid can still sit far apart. A large value therefore establishes real spatial spread, whereas a small value does not establish that a tool\textquotesingle s poses sit together. The supported reading is that AutoDock scatters its in-cluster poses along the radial coordinate. A paired restriction to the 110 complexes defined for all tools keeps AutoDock highest on this statistic, at 0.70\angstrom{} against 0.57\angstrom{} and 0.31\angstrom{}. Appendix~\ref{supplementary-statistics} gives the omnibus and pairwise statistics.

Panel (D) shows median cluster radius increasing from 1.97\angstrom{} for single-tool clusters to 3.75\angstrom{} for three-tool clusters, but the association is no longer detectable after adjustment for pose count (partial \ensuremath{\rho} = \ensuremath{-}0.09, p = 0.12). The partition has a median bootstrap Jaccard of 0.78. At the 4\angstrom{} crystal-site threshold, the retrospective oracle recovers 95\% of complexes, compared with about 57\% for the best inexpensive first-cluster rules. Consensus ranking reaches 57.1\%, its medoid-centred variant 56.8\% and confidence weighting 57.8\%. These three rules are not separable from one another, and each gains about five points on the 52.5\% legacy pose-count rule, although only confidence weighting clears correction against it (Holm-corrected McNemar p = 0.015, against 0.122 for consensus ranking and 0.188 for its medoid-centred variant). The 38-point oracle gap reflects site ranking. The pooled cloud usually contains the crystal-compatible region, but blind prospective selection remains difficult. Figure~\ref{fig:results-r6} in Appendix~\ref{supplementary-statistics} reports internal validity, resampling stability and precision-at-one for all five rules.

Across the full pose cloud, the pooled three-tool oracle places a pose within 4\angstrom{} of the crystal site for 97.7\% of complexes, at a median distance of 0.34\angstrom{}. AutoDock alone reaches 88.1\% at 0.53\angstrom{} and DiffDock 84.5\% at 0.56\angstrom{}. Together they already reach 97.7\% at 0.37\angstrom{}, so EquiBind adds nothing to the pooled reach. Averaging cluster members makes the pooled centre coarser than the best individual pose, with a median distance of 1.17\angstrom{}.
...
EquiBind complexes n & 248 & 239 & 230 & 226 & 217 \\
EquiBind precision & 0.486 & 0.480 & 0.486 & 0.484 & 0.473 \\
EquiBind recall & 0.437 & 0.419 & 0.412 & 0.411 & 0.398 \\
EquiBind F1 & 0.455 & 0.441 & 0.437 & 0.436 & 0.425 \\
\multicolumn{6}{@{}p{\dimexpr\columnwidth-6\tabcolsep\relax}@{}}{\footnotesize%
Each cell is the unweighted mean over the complexes that contribute a PoseBusters-valid pose at that rank, so n varies along a row and the three tools are compared on overlapping rather than identical cohorts.} \\
\end{longtable}
...
EquiBind places poorly on this receptor, and the failure is one of placement rather than geometry. Its poses are covalently sound, with no bond-length or bond-angle failure among the 120 screened and per-ligand validity of 52.5 to 97.5\%. The calibration benchmark behaves the same way, producing no bond-length or bond-angle failures among 27,270 screened EquiBind poses. All 25 bond-length and 22 bond-angle failures among the 72,221 poses from the eight screened variants belonged to DiffDock. AutoDock and EquiBind contributed none. Only two of the 98 valid poses fall outside the slab.
...
Human Orai1 is the pore-forming subunit of the calcium release-activated calcium channel. Its hexameric architecture around a central pore was first resolved in the \emph{Drosophila melanogaster} orthologue, which shares 73\% sequence identity with human Orai1 across the transmembrane region \cite{ref037}. The docked receptor is a homology model of that architecture with human residue numbering (Appendix~\ref{orai1-receptor-model}). It spans residues 66--288 in each subunit, totalling 1,338 residues and 10,362 heavy atoms, without ligand, Ca\textsuperscript{2+} or other heteroatoms. It includes the signature residues R91 and V102 and the E106 selectivity filter. The ensemble comprises four molecular-dynamics snapshots. These are the equilibrated starting structure Fr0 and later frames Fr300, Fr400 and Fr499. The contributing Orai group supplied all four from its structural and molecular-dynamics work \cite{ref038}, \cite{ref072}. Preparation metadata (the modelling program, templates, force field and membrane composition) was not provided and is not reconstructed here. The four frames provide limited ensemble sampling, which partly mitigates but does not remove the rigid-receptor approximation \cite{ref036}. Appendix~\ref{orai1-receptor-model} documents the model, provenance, pocket-detection profile and the one AutoDock search receptor prepared with a permissive residue filter. Its Figure~\ref{fig:results-r10} shows the Fr300 snapshot used throughout this chapter.

The four frames are unevenly distributed. After Kabsch superposition of 1,338 C\ensuremath{\alpha} atoms, Fr0 lies 5.90, 5.88 and 6.02\angstrom{} from Fr300, Fr400 and Fr499, whereas those three differ by only 0.93--1.19\angstrom{}. The ensemble therefore contains one aligned outlier and a tight trio. Orai also differs from the largely soluble calibration benchmark because its top-ranked cavity is the elongated, hydrated ion pore rather than a buried hydrophobic pocket.
...
Physical validity is the per-unit percentage of retained poses that pass every PoseBusters check (Table~\ref{tab:appendix-posebusters-checks}), using all poses after the top-ten cap as the denominator. Invalid, off-receptor and inside-slab poses remain in that denominator. The cap selects more strongly on the experimental panel. Each unit wrote up to thirty poses and retained the tool-ranked top ten, whereas the control panel wrote at most ten and retained all ten when available. Table~\ref{tab:appendix-orai-protocols} records this and the other panel differences.
...
Figure~\ref{fig:results-r11} shows the smallest cross-panel change for AutoDock Vina. Median validity is 100\% in both panels, and the experimental mean is 100.0\% compared with 99.1\% in the benchmark. EquiBind changes most, from a benchmark median of 30\% and mean of 42.4\% to an experimental median of 100\% and mean of 81.7\%. DiffDock also improves, from a benchmark median of 80\% and mean of 78.5\% to an experimental median of 100\% and mean of 93.3\%. Fully valid units further distinguish the tools. Their benchmark rates are 97.0\% for AutoDock, 22.2\% for DiffDock and 16.0\% for EquiBind, rising to 100.0\%, 66.7\% and 58.3\% on the experimental ligands. EquiBind remains bimodal on the benchmark, where its lower quartile is zero and 27\% of units yield no valid pose.

Each tool was compared between panels using an unpaired two-sided Mann-Whitney U test on per-unit yield and Cliff\textquotesingle s \ensuremath{\delta}, where positive values favour the benchmark. DiffDock is cleaner experimentally, with \ensuremath{\delta} = \ensuremath{-}0.49 at the frame-ligand unit and \ensuremath{-}0.68 after collapsing each ligand to the median across four frames. This direction also follows from the selection asymmetry. Experimental units retain the best ten of thirty written poses, whereas control units retain whichever ten were written. The contrast reaches Holm p = 0.006 at the frame-ligand unit but not at the ligand unit (p = 0.114), so it indicates direction and effect size rather than an established difference. AutoDock changes least (\ensuremath{\delta} = \ensuremath{-}0.03 and \ensuremath{-}0.02) and does not approach correction at either unit. EquiBind now moves with DiffDock rather than against it (\ensuremath{\delta} = \ensuremath{-}0.55 and \ensuremath{-}0.69), reaching Holm p = 0.002 at the frame-ligand unit but not at the ligand unit (p = 0.114).

The comparison of three experimental ligands with 308 controls has little power. Non-significance means that no difference was detected, not that none exists. Both EquiBind panels use the same gnina optimisation step and the same preparation settings in Table~\ref{tab:appendix-orai-protocols}. Its cross-panel result therefore reflects ligand-set and pose-selection differences and remains descriptive. The physical-validity ordering is preserved in both Orai panels, consistent with but not proof of transfer from calibration.
...
AutoDock Vina leads on the benchmark but not the experimental ligands. Its medians are 90\% and 60\%, and its means 82.8\% and 54.2\%. DiffDock is lower on the benchmark (median 71.4\%) but higher experimentally (65.0\%), with means of 70.9\% and 68.3\%. EquiBind is degenerate in both panels. Its median and both quartiles are zero, with means of 2.9\% and 1.7\%.

An unpaired two-sided Mann-Whitney U test and Cliff\textquotesingle s \ensuremath{\delta} compare frame-ligand units, removing pose nesting but not correlation among units from one ligand. AutoDock yields less experimentally than on the benchmark (U = 11749.0, \ensuremath{\delta} = +0.59, Holm p = 5.6e-4 across three tools). It is the only contrast that survives at both units and represents all three experimental ligands. The ligand-collapsed medians are 55.0\% and 92.5\% (\ensuremath{\delta} = +0.94, Holm p = 0.012). DiffDock trends in the same direction without significance (U = 8462.0, \ensuremath{\delta} = +0.16, p = 0.667), while EquiBind shows little change (\ensuremath{\delta} = +0.01, p = 0.875).

The pooled pose-level analysis agrees but estimates rate differences rather than differences between per-unit means. These diverge for DiffDock because unit pose counts vary. AutoDock falls by 28.7 percentage points, from 82.83\% to 54.17\%, with a Fisher p below 1e-4 after correction. DiffDock changes by 2.7 points (71.07\% to 68.33\%) and EquiBind by 1.2 points (2.86\% to 1.67\%). Both are non-significant. With twelve correlated experimental units, these null results do not establish equivalence.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image15.png}
\caption[Transmembrane loss of PoseBusters-valid poses]{Transmembrane loss of PoseBusters-valid poses, benchmark against experimental ligands. Panel (A) loss rate per tool as pooled pose-level proportions with Wilson 95\% intervals. Panel (B) the difference per tool as benchmark minus experimental, in percentage points}\label{fig:results-r14}
\end{figure}

Figure~\ref{fig:results-r14} shows the proportion of PoseBusters-valid poses inside the slab under the same top-ten cap as Table~\ref{tab:results-orai-yield}. The denominator is each tool\textquotesingle s valid poses, not a fixed count. AutoDock Vina loses 16.4\% of control and 45.8\% of experimental poses, DiffDock 9.6\% and 26.8\%, and EquiBind 93.3\% and 98.0\%. These pose-level contrasts are descriptive because poses cluster within frame-ligand units. Together with Figure~\ref{fig:results-r13}, they distinguish validity from conditional placement. DiffDock loses less on both panels and therefore has higher experimental usable yield despite AutoDock Vina producing more valid poses. EquiBind almost entirely fails placement.

The per-unit usable-yield test corroborates only AutoDock Vina\textquotesingle s 29.4-point benchmark-to-experimental loss gap, at both the frame-ligand and ligand units. The DiffDock and EquiBind gaps remain pooled descriptions. Absolute differences are preferable to ratios because the experimental denominator contains three ligands and at most twelve units, making the apparent 0.36-fold reduction in loss unstable. Because the four frames sample one receptor, even the per-unit test is anti-conservative for generalisation to new ligands.

\hypertarget{cross-tool-clustering-1}{%
\subsection{Cross-Tool Clustering}\label{cross-tool-clustering-1}}

Clustering was performed for each MD frame, ligand and tool to assess cross-tool localisation and within-tool pose-cloud structure. Frames of one ligand are correlated snapshots of the same receptor-ligand system, so pooled medians are mildly pseudoreplicated. Collapsing each ligand across its frames gives a benchmark median of 29.3\angstrom{}, close to the pooled 28.4\angstrom{}. The experimental estimate changes from a pooled 38.1\angstrom{} to 40.8\angstrom{} after collapse and is therefore unstable, as expected from three ligands and twelve usable pairs. Only the benchmark value is a stable central estimate.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image16.png}
\caption[Distribution of inter-tool consensus-site distances on Orai]{Distribution of inter-tool consensus-site distances on Orai. Step histograms per tool pair, with panel (A) the experimental ligands and panel (B) the benchmark ligands. The dashed vertical line marks the 5\angstrom{} agreement threshold and the dotted lines the per-pair medians}\label{fig:results-r15}
\end{figure}

Figure~\ref{fig:results-r15} shows pairwise consensus-site distances. No experimental pair lies within the 5\angstrom{} threshold. Both benchmark EquiBind pairings are also sparse below it, whereas the tallest AutoDock--DiffDock bin lies just below 5\angstrom{}, despite most of that distribution lying above. Distances extend from a few ångströms to roughly 75\angstrom{} along the elongated pore. Pairwise medians are 26--31\angstrom{} for benchmark and 27--48\angstrom{} for experimental ligands. Continuous distances and pairwise rates are therefore more informative than the binary all-pairs flag, which requires every available pairing in a unit to lie within 5\angstrom{}. This occurs in 108 of 1,204 (9.0\%) multi-tool benchmark units and none of the eleven experimental units.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image17.png}
\caption[Cross-tool agreement on Orai]{Cross-tool agreement on Orai. Panel (A) gives the inter-tool consensus-site distance per tool pair, hatched for the experimental ligands and solid for the benchmark set, and its fourth position summarises all pairs as the mean per pair. Panel (B) gives the percentage of pairs within the 5\angstrom{} threshold per tool pair, with Wilson 95\% intervals and the underlying counts, and its fourth position pools every pair into one rate rather than averaging the three per-pair rates}\label{fig:results-r16}
\end{figure}

Figure~\ref{fig:results-r16} resolves agreement by tool pair. For this endpoint, positive Cliff\textquotesingle s \ensuremath{\delta} means greater separation experimentally, opposite to the benchmark-higher orientation used for yield and cluster quality. The largest gap is DiffDock--EquiBind, at 47.6\angstrom{} experimentally versus 30.7\angstrom{} on the benchmark (\ensuremath{\delta} = +0.91). Although nominally significant at both units, ligand-level collapse retains only two experimental ligands because no Synta-66 EquiBind pose survived the slab filter and top-ten cap. This is an exploratory estimate, not a corrected finding. The other two pairings are flat. AutoDock and DiffDock lie within 5\angstrom{} in 15\% of benchmark pairs but none of eleven usable experimental pairs. EquiBind pairings remain at or below 4\% on the benchmark and zero experimentally. Across all three pairings, 10\% of benchmark pairs and none of seventeen experimental pairs agree. Three-way agreement, which requires all three distances within 5\angstrom{}, occurs in one of 467 benchmark units with all three tools and none of three experimental units. AutoDock--DiffDock is therefore the only material agreement channel.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image18.png}
\caption[Reference-free cluster quality per tool on Orai]{Reference-free cluster quality per tool on Orai, the experimental ligands against the benchmark control panel on the raw pose cloud. Panel (A) silhouette, panel (B) Calinski-Harabasz index, panel (C) Davies-Bouldin index, panel (D) compactness, panel (E) separation and panel (F) bootstrap stability. Panel (B) is truncated on the vertical axis, with 148 outliers off-scale up to a maximum of 2,209,006}\label{fig:results-r17}
\end{figure}

Figure~\ref{fig:results-r17} reports reference-free cluster quality. AutoDock Vina forms the best-separated clouds in both panels and the most compact benchmark clouds. Experimentally, DiffDock is tighter, with a median within-cluster spread of 0.34\angstrom{} versus 0.62\angstrom{} for AutoDock Vina. EquiBind forms the loosest clouds and often one diffuse mode. High bootstrap stability then indicates reproducibility, not accurate site resolution. Among eighteen tool-by-metric cells, the largest cross-panel gap is DiffDock\textquotesingle s silhouette, with medians of 0.17 experimentally and 0.42 on the benchmark (benchmark-higher \ensuremath{\delta} = +0.53). It reaches Holm p = 0.031 at the frame-ligand unit but 0.516 after each ligand is collapsed across four frames and is therefore descriptive. DiffDock\textquotesingle s Calinski-Harabasz result reverses under that collapse. The sensitivity analysis identifies removal of within-ligand spread rather than new evidence. No other difference was detected. With only three experimental ligands, null results neither establish equivalence nor validate the control panel as a substitute.
...
The analysis includes each tool\textquotesingle s top three PoseBusters-valid, outside-slab poses per frame-ligand unit. Both panels use the same four frames, making their residue axes directly comparable. PandaMap assigns residue-level contacts to fifteen interaction classes. After filtering, the experimental panel contains 32 poses from 11 AutoDock complexes and 36 from 12 DiffDock complexes, compared with 3,649 and 3,524 benchmark poses. EquiBind retains two experimental poses and 233 benchmark poses. Its results are non-interpretable and shown only for completeness. Figures display pose-level values, but significance tests first aggregate correlated poses to per-complex means. Contact-count and type-profile comparisons use unpaired Mann-Whitney U tests and Cliff\textquotesingle s \ensuremath{\delta}, with positive values favouring the benchmark.

Experimental poses have descriptively more contacts. The median rises from 26 to 30 for AutoDock Vina and from 15 to 18 for DiffDock. Neither difference survives per-complex testing, and collapsing each ligand across four frames weakens both further. EquiBind\textquotesingle s two usable experimental poses are untestable and excluded from the correction family. No total-contact difference is detected at the available power, which does not establish equivalent contact intensity. Figure~\ref{fig:results-r19} in Appendix~\ref{orai1-panels} reports the distributions and four effect sizes.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.89\textheight,keepaspectratio]{media/media/image21.png}
\caption[Interaction-type profile on Orai]{Interaction-type profile on Orai. Mean count per pose of each PandaMap interaction type, with panel (a) AutoDock Vina, panel (b) DiffDock and panel (c) EquiBind. Solid bars are the benchmark ligands and hatched bars the experimental ones. The ionic, salt-bridge and attractive-charge classes are drawn as a single charged bar, because the profiler writes every charged contact once under each of the three. This figure and Figure~\ref{fig:results-r22}, together with Figure~\ref{fig:results-r21} in the Appendix, carry lower-case panel letters, as printed on the panels themselves}\label{fig:results-r20}
\end{figure}

Figure~\ref{fig:results-r20} reports mean interaction counts per pose. Alkyl-\ensuremath{\pi}, hydrophobic, other \ensuremath{\pi} and hydrogen-bond interactions dominate under both mature tools, but their order changes. Hydrogen bonds rank second on the benchmark and fifth experimentally. Benchmark ligands form more hydrogen bonds (Cliff\textquotesingle s \ensuremath{\delta} = +0.42 for AutoDock and +0.54 for DiffDock) and all ionic, salt-bridge and attractive-charge contacts. These charged classes are absent from both tools\textquotesingle s modulator poses. Modulators are more hydrophobic (\ensuremath{\delta} = \ensuremath{-}0.64 and \ensuremath{-}0.58) and richer in alkyl-\ensuremath{\pi} contacts (\ensuremath{-}0.45 and \ensuremath{-}0.59). They also register more halogen bonds, but that result carries no weight because the profiler is blind to chlorine and bromine and misses chlorine in part of the control library (Appendix~\ref{interaction-fingerprints}).

These patterns remain descriptive. At the frame-ligand unit, thirteen of twenty-six tool-by-type cells pass Benjamini-Hochberg correction. Two are the discounted halogen-bond cells, leaving eleven interpretable cells. None passes at the ligand unit, where the smallest adjusted values are 0.086 and 0.106. Table~\ref{tab:appendix-orai-interaction} gives the corresponding mean counts. The common direction is chemically plausible because the modulators are halogenated and lipophilic, whereas the benchmark library is more polar and hydrogen-bond driven, but it is not an established panel difference.

Both panels concentrate contacts on Asp110, Gln108, Asp112, Ala111, Tyr115, His113, Asp114, Pro201 and Leu202. No residue differs after Holm correction of the fifteen within-tool tests. Figure~\ref{fig:results-r21} in Appendix~\ref{orai1-panels} gives the residue profiles.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image23.png}
\caption[Contact-fingerprint overlap between the two ligand sets]{Contact-fingerprint overlap between the two ligand sets. Panel (a) counts the characteristic Orai1 residues, those contacted in at least 10\% of a tool\textquotesingle s poses, split into shared, benchmark-only and experimental-only. Panel (b) gives the residue-level and typed Jaccard overlap per tool}\label{fig:results-r22}
\end{figure}

Figure~\ref{fig:results-r22} compares contacted residues between panels. Residue-level Jaccard overlap is 0.42 for AutoDock and 0.31 for DiffDock, falling to typed-contact values of 0.40 and 0.27. Because all values use the same 10\% characteristic-contact threshold, these declines reflect changed interaction types rather than a changed basis. AutoDock shares 29 characteristic residues and DiffDock 13. EquiBind shares 5, but its overlap is uninterpretable from two experimental poses. The 10\% threshold also corresponds to roughly four experimental poses but several hundred benchmark poses. Diverse benchmark ligands distribute contacts broadly, whereas three repeatedly docked modulators concentrate them. The larger experimental-only count therefore reflects set size and homogeneity, not promiscuity. Shared residues and per-residue frequencies are more robust.

The surviving modulator poses partially agree with published pharmacology. Synta-66 maps to an extracellular TM1/TM3 region and adjoining loops near the E106 selectivity filter \cite{ref071}. Its defining extracellular loop1 and loop3 residues (His113, Tyr115, Pro201 and Leu202) lie above the filter and are characteristic of both tools\textquotesingle{} modulator poses. Leu109 is characteristic only of AutoDock Vina in both panels. Phe199 is characteristic of AutoDock Vina in both panels and of DiffDock only experimentally, where 19.4\% of poses contact it compared with 6.3\% on the benchmark. Each tool produces one modulator pose contacting Glu106, so neither consistently recovers the filter residue. These contacts support biological interpretation and hypotheses for mutagenesis or ligand-bound simulation, but evidence from different tools cannot be combined to validate one binding mode.

Because the panels are unpaired and differ in chemistry, contact types characterise chemotypes rather than tool skill. Literature concordance is partial without a cognate structure, and some retained poses remain anatomically implausible. Residue tests are pose-level and pseudoreplicated, the experimental panel contains three ligands, and EquiBind contributes two usable poses. Effect sizes and qualitative pharmacological agreement therefore outweigh nominal p-values.

Net usable yield reverses the control-panel ordering. AutoDock Vina leads on controls, while DiffDock leads experimentally. Calibration point estimates show the same ordering, although not a statistically separable difference. EquiBind loses 96 of 98 valid poses to the exclusion rule and retains two of 120.

Three conclusions follow. First, physical validity alone overstates utility on a channel because a geometrically valid pose may occupy the ion-conduction pathway. Usable yield exposes EquiBind\textquotesingle s placement failures. Second, cross-tool consensus provides no reliable confidence signal. Median separation is roughly 30\angstrom{} on the benchmark and at least as large experimentally, although the experimental median is unstable after frame-robust collapse. At best 14\% of pairs agree within 5\angstrom{}. Six-fold symmetry does not explain the scatter. Folding each separation onto the nearest symmetry copy moves the benchmark median only from 29.5 to 27.3\angstrom{} and leaves every experimental pair above the threshold. Appendix~\ref{orai1-panels} gives the axial and radial decomposition behind that result. All three tools agree in one of 467 benchmark units and no experimental unit. Filtering valid, outside-slab poses does not detectably tighten the scatter, but without an equivalence test this only bounds the observed filtering effect.
...
Cross-panel agreement is limited. Of fifty-six contrasts, fifteen have adjusted p below 0.05 at the frame-ligand unit and three at the ligand unit. The sensitivity analysis identifies one of the latter, DiffDock\textquotesingle s Calinski-Harabasz index, as a collapse artefact. AutoDock Vina\textquotesingle s lower experimental usable yield is the only result significant at both units and supported by all three ligands. It is the strongest cross-panel observation but remains descriptive because three ligands and four snapshots of one receptor cannot support general inference. For Orai1 the practical recommendation is therefore to retain AutoDock Vina and refined DiffDock as separate hypothesis generators, to prioritise poses that satisfy the validity and placement rules and agree with independent pharmacology, and to omit EquiBind in its present configuration. Sparse overlap between the two viable tools should not itself be treated as validation.

\hypertarget{pose-production-costs}{%
\section{Pose Production Costs}\label{pose-production-costs}}

Cost is normalised per qualifying pose, defined retrospectively as PoseBusters-valid and within 2\angstrom{} of the crystal ligand. All docking used one workstation, whose specification and stage-specific devices are given in Appendix~\ref{docking-protocols}.

All figures use the whole-protein run of 303 complexes and include both timed AutoDock stages. The blind Vina search at exhaustiveness 128 took 4.73 wall-hours and gnina rescoring consumed 10.27 GPU-hours of device occupancy, making the rescorer roughly twice as expensive as the search it re-ranks. AutoDock therefore required 15.00 charged hours, 68.5\% for rescoring. Per-pose values divide this total.\footnote{Cost is charged as device occupancy throughout, which is the only currency comparable across these pipelines and the one the published DiffDock and EquiBind figures already use. Search and rescoring consume 151.42 CPU-core-hours and 10.27 GPU-hours, which cannot be added; the CPU value is the search wall multiplied by 32 requested threads, not measured occupancy. The rescoring pass was run sixteen ways concurrently and finished in 0.73 wall-hours, so its wall clock is a scheduling result rather than a property of the method. Charging it at wall clock instead would put AutoDock at 5.46 total wall-hours with 13.4\% for rescoring, and would flatter this arm against the serially-timed pipelines it is compared with. The concurrency was measured to give a 9.5-fold throughput gain on an identical workload, so it changes elapsed time without changing the work done.}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image24.png}
\caption[Wall-clock seconds per qualifying pose]{Wall-clock seconds per qualifying pose, where a qualifying pose is PoseBusters-valid and within 2\angstrom{} of the crystal ligand. Each box summarises only the complexes for which that tool produced a qualifying pose, namely 199, 169 and 62, whereas the bracketed paired tests use the 44 complexes on which all three succeed}\label{fig:results-r23}
\end{figure}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1200}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1400}}@{}}
\caption[Cost per qualifying pose under the blind whole-protein search]{Cost per qualifying pose under the blind whole-protein search, where a qualifying pose is PoseBusters-valid and within 2\angstrom{} of the crystal ligand. Wall-clock medians and interquartile ranges are taken over the complexes for which that tool produced a qualifying pose. The two hardware currencies are pooled aggregates and are never summed}\label{tab:results-cost-per-qualifying-pose}\tabularnewline
\toprule\noalign{}
Pipeline & \shortstack[r]{Wall-clock\\ median (s)\strut} & \shortstack[r]{Interquartile\\ range (s)\strut} & Complexes & \shortstack[r]{CPU-core-\\ seconds\strut} & \shortstack[r]{GPU-\\ seconds\strut} & \shortstack[r]{Qualifying\\ poses\strut} \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
Pipeline & \shortstack[r]{Wall-clock\\ median (s)\strut} & \shortstack[r]{Interquartile\\ range (s)\strut} & Complexes & \shortstack[r]{CPU-core-\\ seconds\strut} & \shortstack[r]{GPU-\\ seconds\strut} & \shortstack[r]{Qualifying\\ poses\strut} \\
...
DiffDock & 50.5 & 14.5 to 132.5 & 169 & 2.93 & 49.9 & 1,622 \\
EquiBind & 8.5 & 3.2 to 22.1 & 62 & 849.1 & 2.1 & 305 \\
\midrule\noalign{}
AutoDock, Vina search only & 21.7 & 9.9 to 45.3 & 202 & 1,805.0 & 0.0 & 302 \\
\multicolumn{7}{@{}p{\dimexpr\columnwidth-7\tabcolsep\relax}@{}}{\footnotesize%
The first three rows show each family\textquotesingle s selected variant. The final row removes the separately timed gnina pass from AutoDock and therefore decomposes the measured campaign rather than representing another run. Its CPU-core-second numerator still covers only Vina. Its per-pose CPU value is the higher of the two because rescoring adds nine qualifying poses at no CPU cost, raising the denominator from 302 to 311.} \\
\end{longtable}

The configured AutoDock pipeline is the most expensive route to a qualifying pose under blind whole-protein search. Figure~\ref{fig:results-r23} shows the per-complex wall-clock distributions, and Table~\ref{tab:results-cost-per-qualifying-pose} reports all three currencies. Among 44 complexes where every tool succeeds, the ordering persists (Friedman \ensuremath{\chi}\textsuperscript{2} = 59.5, p = 1.2e-13, Kendall W = 0.68), and every pair passes Holm correction. Median per-complex cost ratios are 0.14 for DiffDock versus AutoDock and 0.06 for EquiBind. gnina raises AutoDock cost to roughly three times the search alone, then that total is amortised over few qualifying poses. The median successful complex yields one, although 199 of 303 complexes now yield at least one, far more than the 169 of DiffDock.

Tightening the endpoint increases cost most for AutoDock. Median seconds per generated, valid, and valid-near-native pose are 5.3, 5.3 and 137.2 for AutoDock. The DiffDock values are 8.1, 9.8 and 50.5, and the EquiBind values are 0.9, 1.2 and 8.5. Contributing complexes fall from 303 to 301 to 199 for AutoDock, 303 to 300 to 169 for DiffDock, and 303 to 248 to 62 for EquiBind. Moving from valid to valid-near-native multiplies unit cost by about 25.8, compared with 5.2 for DiffDock. The multiplier is large for AutoDock because a complex that succeeds contributes only one or two qualifying poses out of thirty, not because it succeeds rarely. EquiBind\textquotesingle s 8.5 s median describes only the 62 successful complexes. It fails in the other 241.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image25.png}
\caption[Hardware resource per qualifying pose]{Hardware resource per qualifying pose, defined as in Figure~\ref{fig:results-r23}}\label{fig:results-r24}
\end{figure}

Removing parallelism from the resource axis widens the separation (Figure~\ref{fig:results-r24} and Table~\ref{tab:results-cost-per-qualifying-pose}). Configured AutoDock is most expensive on every axis. Its 32 threads compress 151.42 CPU-core-hours of search into 4.73 wall-hours. The subsequent GPU rescoring pass adds device occupancy without adding CPU cost. The tools convert their compute into 311, 1,622 and 305 qualifying poses, which explains much of the gap. GPU time covers docking for DiffDock and EquiBind but only gnina rescoring for AutoDock. AutoDock nevertheless has the highest GPU cost per qualifying pose because its qualifying yield is low.

Four caveats apply. First, wall-clock time is cross-hardware and device mixes differ within and between pipelines. AutoDock spends 4.73 of its 15.00 charged hours on a multithreaded CPU and 10.27 on a GPU shared by up to sixteen concurrent workers, so elapsed time does not measure identical work. Second, resource values are order-of-magnitude estimates. AutoDock CPU-core-seconds cover only Vina and assume all 32 threads are occupied, while gnina is recorded separately as GPU occupancy. Third, part of the AutoDock rescoring pass is an implementation artefact. The rescorer was invoked once per pose, and each invocation paid the start-up of a separate process on a GPU that a single-pose evaluation leaves substantially idle. Batching would therefore reduce the cost of the learned rescorer by an amount this run does not determine, since no batched implementation was measured. Fourth, EquiBind wall time is an idealised best-configuration estimate, calculated as selected-configuration compute-seconds divided among 32 configured workers. It assumes perfect overlap, unlike the measured AutoDock and DiffDock elapsed times. This convention separates EquiBind\textquotesingle s resource and wall axes. Its 71.94 CPU-core-hours are under half of AutoDock\textquotesingle s 151.42, and its 2.25 wall-hours are about a seventh of AutoDock\textquotesingle s 15.00 charged hours. Vina already uses all 32 threads per docking, whereas EquiBind is credited with concurrency across complexes. The pooled resource panel has no inferential test, but pipeline success differs strongly at the complex level (Cochran\textquotesingle s Q = 100.3, p = 1.7\ensuremath{\times}10\textsuperscript{\ensuremath{-}22}).

Under the blind whole-protein configuration used throughout this chapter, configured AutoDock is most expensive per qualifying pose in wall-clock, CPU and GPU terms. This does not apply to the classical search alone. Without gnina, Vina requires a median 21.7 s and no GPU time per qualifying pose, still below DiffDock on wall-clock. Its CPU cost per qualifying pose is 1,805.1 s, higher than the rescored arm\textquotesingle s 1,752.8 s, because rescoring adds nine qualifying poses at no CPU cost. Vina remains by far the most processor-intensive engine on either arm, while AutoDock becomes most expensive in wall and GPU time only after learned rescoring. It is also not cheapest per generated pose on any currency (Table~\ref{tab:appendix-cost-per-pose}). Its advantage is coverage and admissibility, not economy.

The ordering depends on the charged unit. Median wall-clock time per attempted docking is 157.7 s for AutoDock, 241.1 s for DiffDock and 25.6 s for EquiBind. Because each produces about thirty poses per complex, this rescales the per-complex median rather than providing an independent measure. Allocating the whole campaign to complexes that yield a qualifying pose gives 271, 507 and 131 s. On this denominator AutoDock is cheaper than DiffDock in wall-clock and GPU terms, with 186 versus 478 GPU-seconds per success, but remains much more expensive in CPU-core-seconds. Neither denominator changes the per-qualifying-pose result.
...
This study compared AutoDock Vina, DiffDock and EquiBind by crystallographic re-docking of 303 calibration complexes and reference-free docking of three Orai1 modulators into four molecular-dynamics snapshots. The calibration benchmark provides stronger evidence because each prediction has an experimental reference. Its endpoints do not identify one winner. AutoDock Vina had the highest physical validity, while refined DiffDock had the highest headline point estimate and matched or exceeded AutoDock on native-pocket recovery at each ranking depth under the blind whole-protein search. The DiffDock near-nativeness advantage remains inconclusive. The paired intervals permit both no difference and a lead of roughly thirteen percentage points. Refined EquiBind had low raw validity, in-place accuracy and pocket localisation. Orai1 preserved the validity ordering but reversed the usable-yield ordering. Without a ligand-bound structure, neither result identifies the correct binding mode.
...
Refiner selection remains an empirical pipeline choice. DiffDock + smina led gnina, and unguided EquiBind + gnina led smina, but neither within-family difference was statistically separable. The study measured neither optimisation trajectories nor refiner performance on an independent set, so proposed mechanisms such as gentle preservation or aggressive repair remain interpretations.
...
Ranking depth separates generation from selection. Moving from rank-1 to the best of fifteen substantially increased recovery for DiffDock and AutoDock but added only about five percentage points for EquiBind (Results, Near-Nativeness and Form). Useful DiffDock and AutoDock poses were therefore often under-ranked. Selection was not the main loss. About seven in ten rank-1 failures of either tool contained no qualifying pose at any depth. The corresponding share for EquiBind was about nine in ten. Re-ranking cannot recover an absent binding mode.

The form decomposition supports this result. At the prespecified 1\angstrom{} Kabsch threshold, DiffDock had the highest point estimate for both internal form and near-nativeness, although its near-nativeness advantage over AutoDock remained inconclusive. DiffDock often generated a credible conformation without ranking it first. EquiBind was weaker on both axes. A ligand can retain its shape while presenting its pharmacophore to the wrong residues, so form and placement must remain distinct.

Pocket recovery was easier than exact pose recovery under the whole-protein search. AutoDock and DiffDock intervals overlapped at rank-1. At top-15 DiffDock had the higher point estimate and the paired reach test reported in the calibration Cross-Tool Clustering section separated them. Pooling all tools placed the crystal pocket somewhere in the pose cloud for 93\% of complexes, but the best inexpensive cluster rule ranked it first in only 55\%. This is a site-selection gap, not 93\% exact-pose accuracy. Cluster membership is coarser than a valid pose within 2\angstrom{}.
...
Orai1 is more realistic but less identifiable than crystallographic re-docking. AutoDock Vina produced valid poses for nearly all three modulators but retained a smaller share after the combined on-receptor and transmembrane-placement screen. Refined DiffDock had lower validity but more usable poses. EquiBind retained two of 120. Without an experimental ligand pose, these rates measure admissibility rather than near-nativeness.
...
Within the whole-protein configuration, EquiBind had the shortest median wall-clock cost per qualifying pose, followed by DiffDock and AutoDock, but the accounting differs. AutoDock and DiffDock use measured elapsed time. EquiBind uses an idealised best-configuration estimate and succeeds on only 62 of 303 complexes, compared with 169 for DiffDock. The ordering is therefore firmer than its magnitude. Within this protocol, configured AutoDock was the most expensive route on the wall-clock and hardware axes. Its return was coverage and validity rather than speed.
...
The results support a staged workflow that generates several hypotheses, optimises local geometry, rejects invalid or biologically inadmissible structures, ranks the remainder with an endpoint-specific selector and retains uncertainty when tools disagree. Refined DiffDock and AutoDock Vina are complementary. DiffDock has higher accuracy and pocket-localisation point estimates, while AutoDock supplies safer geometry. Their near-nativeness difference remains inconclusive. EquiBind provides too little coverage for routine use in this configuration. Its Orai1 control panel yielded only 2.9\% usable poses, compared with 82.8\% for AutoDock and 71.1\% for DiffDock.
...
All pipelines searched the whole receptor, and the headline metric required PoseBusters validity and an RMSD within 2\angstrom{}. Rank-1 recovery was 36.6\% for AutoDock, 34.0\% for DiffDock and 15.2\% for EquiBind. Retrospective top-15 recovery was 65.3\%, 55.1\% and 20.1\%. The AutoDock--DiffDock rank-1 difference was +2.6 percentage points (95\% interval \ensuremath{-}4.2 to +9.4) and was not statistically resolved, although AutoDock led from top-5 onwards, by +13.2 points there (p\_holm = 7.6e-4) and +10.2 at top-15. That ordering is conditional on search effort. AutoDock ran at exhaustiveness 128, the top rung of a five-value ladder, and no measurement here speaks to what a harder search would return (Appendix~\ref{exhaustiveness-selection}). Post-hoc 90\% compatibility intervals place the difference within about twelve points. They are descriptive rather than prespecified equivalence tests. Both leading tools remained decisively ahead of EquiBind.

Physical validity separated the tools in the same order, at 98.9\%, 84.5\% and 51.7\%, although part of this separation is constitutive (Appendix~\ref{validity-objective-coupling}). Rank-1 native-interaction recovery likewise did not separate the leading tools. DiffDock led only for the top-five contact union. On Orai1, median usable yield under the study\textquotesingle s placement rule was 65\% for DiffDock, 60\% for AutoDock and 0\% for EquiBind. This untested reversal rests on twelve correlated units from three ligands and is descriptive. Only two of 120 EquiBind poses passed both screens. The separate control panel retained the same ordering.

Stability here means static validity, clustering reproducibility and frame sensitivity. Docking did not test thermodynamic or time-dependent stability.
...
Local optimisation mainly repaired geometry. DiffDock validity rose from 24.4\% raw to 84.5\% with smina and 87.1\% with gnina. Unguided EquiBind rose from 2.9\% to 39.2\% and 51.7\%. Fixed-rank near-native rates changed by only 0.1--0.7 percentage points for DiffDock, 2.8--3.1 for EquiBind with gnina and 2.5--2.6 for EquiBind with smina. The larger cumulative top-\emph{k} gains in Table~\ref{tab:appendix-refinement-mcnemar} accumulate these small repairs across ranks, and gnina also re-ranks EquiBind poses. Local optimisation therefore made many learned poses admissible but could not supply an absent binding mode or correct gross placement.

\hypertarget{rq3-computational-efficiency}{%
\section{RQ3: Computational Efficiency}\label{rq3-computational-efficiency}}

Across 303 whole-protein searches, median wall-clock cost per qualifying pose was 8.5 s for EquiBind, 50.5 s for DiffDock and 119.7 s for AutoDock. DiffDock succeeded on 169 complexes, AutoDock on 199 and EquiBind on 62. AutoDock and DiffDock times were measured. The 8.5 s EquiBind value idealises perfect concurrency for one configuration. The measured 31.3 s full-run value covers all nine EquiBind configurations, three site modes each taken raw, smina-optimised and gnina-optimised (Appendix~\ref{equibind-pocket-guided}), including 38 complexes for which P2Rank returned fewer than three pockets. The single-configuration cost lies between these values. EquiBind retains the shortest time, but its estimated advantage over DiffDock falls from sixfold to 1.6-fold.

Configured AutoDock was most expensive per qualifying pose in wall-clock, CPU-core and GPU seconds. This does not apply to Vina alone. Removing gnina puts its wall-clock and GPU cost below DiffDock, although Vina retains the highest CPU-core cost. The 2.22-hour blind search and 12.17-hour gnina pass total 14.38 measured wall-hours, 84.6\% of which belongs to rescoring. Per-pose process start-up leaves the GPU underused, so batching should reduce this cost by an unmeasured amount. These results describe the implementations and hardware, not method classes.
...
AutoDock Vina had the highest PoseBusters pass rate and DiffDock the highest in-place point estimates, but their near-nativeness difference remained inconclusive. Generation and localisation dominated failure. About seven in ten AutoDock and DiffDock rank-1 failures, and nine in ten EquiBind failures, contained no qualifying pose. Selection still recovered 62 and 64 additional complexes at top-15 for the leading tools, compared with 15 for EquiBind. The result is a workflow, not a universal winner.
