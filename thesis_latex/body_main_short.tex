...
All figures use the whole-protein run of 303 complexes. The blind Vina search at exhaustiveness 128 took 4.73 wall-hours. The gnina rescoring pass ran sixteen poses at a time on one GPU and summed 10.27 GPU-hours across those workers, while the device itself was busy for 0.71 hours. Charged on the summed figure AutoDock required 15.00 hours, 68.5\% of it rescoring. Charged on the measured occupancy it required 5.44 hours, 13.1\% of it rescoring. This chapter reports the summed figure throughout, and the cost ordering below is conditional on that choice. Appendix~\ref{hardware-and-timing-basis} sets out both. Per-pose values divide the reported total.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image24.png}
\caption[Charged seconds per qualifying pose]{Charged seconds per qualifying pose. The panel title predates the accounting note in Appendix~\ref{hardware-and-timing-basis}. Its AutoDock bar charges 10.27 hours summed across sixteen concurrent gnina workers rather than the 0.71 hours the pass occupied the device. The bracketed AutoDock and DiffDock test therefore holds on that basis and not on the measured one. Each box covers that pipeline\textquotesingle{}s own 214, 183 and 83 qualifying complexes, while the bracketed tests use the 54 all three share}\label{fig:results-r23}
\end{figure}
