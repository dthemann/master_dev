...
The headline figure is charged device occupancy, defined for every pipeline as CPU-core-seconds divided by the 32 hardware threads of the workstation plus GPU-seconds. Dividing by the thread count converts a saturating processor stage into the time it occupied the machine. The GPU term is the time a GPU stage occupied its device. For the one stage that ran many processes at once on one device, the AutoDock rescoring pass, that time is the measured union of the processes\textquotesingle{} intervals rather than their sum. The processor cores those processes held are charged as CPU-core-seconds. No concurrent stage is therefore charged for more time than its device supplied, and consecutive stages sum.
...
% equibind_p2rank_smina / _gnina) were removed; they live in Appendix \ref{equibind-2}.
...
DiffDock rises from a 13.3\% raw median to 93.3\% with smina, a Hodges-Lehmann gain of +62.1 percentage points. Unguided EquiBind rises from 0\% to 80.0\% with gnina. These near-unanimous gains survive Holm correction, with matched-pairs rank-biserial effects of 0.998--1.000 and adjusted p-values \textless{} 10\textsuperscript{-35}. DiffDock behaves similarly with gnina (96.6\%), whereas EquiBind performs worse with smina (56.7\%). The refiners are thus nearly interchangeable for DiffDock, while gnina is decisive for EquiBind.
...
\multicolumn{6}{@{}p{\dimexpr\columnwidth-6\tabcolsep\relax}@{}}{\footnotesize%
Recovery is the PB-valid, within-2\angstrom{} recovery also reported across distance thresholds in Table~\ref{tab:results-near-native-form}. Gain columns give the added complexes first and the change in recovery share in percentage points in brackets.} \\
\end{longtable}
...
AutoDock and DiffDock have almost fully overlapping reach intervals at every depth, while EquiBind trails both. From rank-1 to top-15, DiffDock reach rises from 222 to 261 of 303 complexes, AutoDock from 237 to 272 and EquiBind from 138 to 155, gains of 12.9, 11.6 and 5.6 percentage points. Pairings become less redundant with depth but remain positively associated. At top-15, all three pairings lie at \ensuremath{\phi} = +0.13 to +0.22 with Fisher p \ensuremath{\leq} 0.036.
...
The three reach rows are percentages of the 303 complexes, rounded to whole percent and carried with Wilson 95\% intervals. Differences taken from the rounded rows can differ from the unrounded gains in the text by a few tenths of a point.} \\
\end{longtable}
...
Net usable yield reverses the control-panel ordering under the output order of each tool. AutoDock Vina leads on controls, while DiffDock leads experimentally. Whether that reversal survives matching the ranking cap depends on the basis of comparison. Restricting AutoDock to Vina modes one to ten matches the selection rule of the control panel. That restriction raises its per-unit median from 60.0 to 75.0\%, above the 65.0\% median of DiffDock, and its ligand-collapsed median from 55.0 to 70.0\% as reported above. Its pooled usable yield rises from 54.2 to 65.8\% and stays below the pooled 68.3\% of DiffDock in Table~\ref{tab:results-orai-yield}. Appendix~\ref{orai1-panels} reports the prefix-matched restriction. Calibration point estimates at rank-1 also put AutoDock ahead, at 49.2\% against 43.6\%, although that difference is not statistically separable. EquiBind loses 106 of 107 valid poses to the exclusion rule and retains one of 120.
...
All figures use the whole-protein run of 303 complexes. The blind Vina search at exhaustiveness 128 took 4.73 wall-hours. The gnina rescoring pass ran sixteen poses at a time on one GPU and cost 1.39 charged hours, 0.71 of them GPU occupancy and the rest the 21.6 CPU-core-hours its workers held. AutoDock therefore required 6.12 charged hours, 22.6\% of them for rescoring. The rescorer cost roughly three tenths of the search it re-ranks. Appendix~\ref{hardware-and-timing-basis} sets out the measurement. Per-pose values divide this total.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image24.png}
\caption[Charged seconds per qualifying pose]{Charged device-occupancy seconds per qualifying pose, with the AutoDock rescoring pass charged at its measured device occupancy.}\label{fig:results-r23}
\end{figure}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1200}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1400}}@{}}
\caption[Cost per Qualifying Pose]{Cost per Qualifying Pose. Charged medians and interquartile ranges are taken over the complexes for which that pipeline produced a qualifying pose. The CPU-core and GPU columns are pooled ratios of campaign totals to qualifying poses, and the GPU column is measured device occupancy except for EquiBind, whose inference seconds are a pool sum (Appendix~\ref{hardware-and-timing-basis}).}\label{tab:results-cost-per-qualifying-pose}\tabularnewline
\toprule\noalign{}
...
AutoDock and DiffDock are not separable in cost per qualifying pose under blind whole-protein search, and EquiBind is far cheaper than either. Figure~\ref{fig:results-r23} shows the per-complex charged distributions, and Table~\ref{tab:results-cost-per-qualifying-pose} reports all three currencies. Among 54 complexes where every tool succeeds, the ordering against EquiBind persists (Friedman \ensuremath{\chi}\textsuperscript{2} = 73.4, p = 1.2e-16, Kendall W = 0.68) and both of its pairs pass Holm correction. The AutoDock and DiffDock pair does not (Holm p = 0.91). Median per-complex cost ratios are 0.56 for DiffDock versus AutoDock and 0.08 for EquiBind. gnina raises the AutoDock campaign to roughly 1.3 times the search alone and the median cost per qualifying pose from 17.8 s to 28.8 s, because that total is amortised over few qualifying poses. The median successful complex yields two, although 214 of 303 complexes now yield at least one, far more than the 183 of DiffDock.
...
Interaction fingerprints show the same dependence on endpoint and depth. DiffDock had the highest mean crystal overlap for the union of top-5 contacts, but the leading tools were not separable at rank-1. This is inconclusive rather than equivalent. The one equivalence statement in this work covers the valid-near-native recovery contrast, and its margin was read off the data after the fact rather than prespecified. AutoDock contact recovery declined through its top five while DiffDock remained flat, and the rankers of both tools were associated with interaction recovery after correction. The association was much stronger for AutoDock, at a median Kendall \ensuremath{\tau} of \ensuremath{-}0.40 against \ensuremath{-}0.11 for DiffDock (Appendix~\ref{rank-quality-concordance}), and the DiffDock association was detected only under the nearest-copy scoring of the Methods. AutoDock therefore concentrated the better poses of a pool whose contact recovery fell away below rank-1, and EquiBind recovered no more native contacts than it missed at any rank, although standardising rank-1 recovery to a common placement distribution puts its F1 at 0.628 against 0.647 for DiffDock (Appendix~\ref{interaction-fingerprints}). External scores require validation for their intended task \cite{ref067}.
...
Across 303 whole-protein searches, median charged cost per qualifying pose was 2.4 s for EquiBind, 28.8 s for AutoDock and 39.0 s for DiffDock. The AutoDock and DiffDock pair is not separable on the 54 complexes all three solve. DiffDock succeeded on 183 complexes, AutoDock on 214 and EquiBind on 83. All three are charged on one basis, CPU-core-seconds divided by the 32 hardware threads of the workstation plus GPU-seconds, with the concurrent AutoDock rescoring pass charged at its measured device occupancy, so the figures are comparable in basis as well as in scope. Every tool is charged for the configuration carried forward and for nothing else, so the selection work each pipeline discarded on the way to that configuration is excluded on all three arms alike. The sixteenfold EquiBind advantage over DiffDock is therefore not an artefact of asymmetric charging, though it is conditional on EquiBind succeeding on a quarter of the set and on a thread occupancy it reaches only across separate complexes.

Configured AutoDock was most expensive per qualifying pose in CPU-core seconds, by nearly an order of magnitude, and DiffDock in GPU seconds. This holds for the rescored arm and for Vina alone, which retains the highest CPU-core cost. The 4.73-hour blind search and the 1.39-hour rescoring pass, 0.71 hours of GPU occupancy plus 21.6 CPU-core-hours at the 32-thread rate, total 6.12 charged hours, 22.6\% of which belongs to rescoring. Per-pose process start-up leaves the GPU underused, so batching should reduce this cost by an unmeasured amount. These results describe the implementations and hardware, not method classes.
