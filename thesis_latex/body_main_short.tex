...
The headline figure is charged cost, defined for every pipeline as CPU-core-seconds divided by the 32 hardware threads of the workstation plus GPU-seconds. Dividing by the thread count converts a saturating processor stage into the time it occupied the machine. The GPU term is the summed elapsed time of the GPU stages themselves. Where such a stage ran many processes at once on one device, as the AutoDock rescoring pass did, that sum exceeds the time the device was busy and the term measures work rather than occupancy. Charging this way stops work that merely ran with more concurrency from appearing cheaper, and it charges a concurrent stage for more time than one device could deliver.
...
All figures use the whole-protein run of 303 complexes. The blind Vina search at exhaustiveness 128 took 4.73 wall-hours. The gnina rescoring pass ran sixteen poses at a time on one GPU and summed 10.27 GPU-hours across those workers, while the device itself was busy for 0.72 hours. Charged on the summed figure AutoDock required 15.00 hours, 68.5\% of it rescoring. Charged on the measured occupancy it required 5.45 hours, 13.2\% of it rescoring. This chapter reports the summed figure throughout, and the cost ordering below is conditional on that choice. Appendix~\ref{hardware-and-timing-basis} sets out both. Per-pose values divide the reported total.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{media/media/image24.png}
\caption[Charged seconds per qualifying pose]{Charged seconds per qualifying pose. The panel title predates the accounting note in Appendix~\ref{hardware-and-timing-basis}. Its AutoDock bar charges 10.27 hours summed across sixteen concurrent gnina workers rather than the 0.72 hours the pass occupied the device. The bracketed AutoDock and DiffDock test therefore holds on that basis and not on the measured one. Each box covers that pipeline\textquotesingle{}s own 214, 183 and 83 qualifying complexes, while the bracketed tests use the 54 all three share}\label{fig:results-r23}
\end{figure}
...
\bottomrule\noalign{}
\endlastfoot
AutoDock & 91.8 & 59.2 to 153.7 & 214 & 1,233.3 & 83.6 & 442 \\
...
On the summed basis the configured AutoDock pipeline is the most expensive route to a qualifying pose under blind whole-protein search. Figure~\ref{fig:results-r23} shows the per-complex charged distributions, and Table~\ref{tab:results-cost-per-qualifying-pose} reports all three currencies. Among 54 complexes where every tool succeeds, the ordering persists (Friedman \ensuremath{\chi}\textsuperscript{2} = 87.1, p = 1.2e-19, Kendall W = 0.81), and every pair passes Holm correction. Charging the rescoring pass at its measured device occupancy instead leaves the AutoDock and DiffDock pair unseparated, while the other two pairs still clear correction. Median per-complex cost ratios are 0.15 for DiffDock versus AutoDock and 0.02 for EquiBind. gnina raises AutoDock cost to roughly three times the search alone on this basis, then that total is amortised over few qualifying poses. The median successful complex yields two, although 214 of 303 complexes now yield at least one, far more than the 183 of DiffDock.

Tightening the endpoint increases cost most for AutoDock. Median charged seconds per generated, valid, and valid-near-native pose are 5.3, 5.3 and 91.8 for AutoDock. The DiffDock values are 7.6, 9.2 and 39.0, and the EquiBind values are 0.3, 0.4 and 2.4. Contributing complexes fall from 303 to 301 to 214 for AutoDock, 303 to 300 to 183 for DiffDock, and 303 to 266 to 83 for EquiBind. Moving from valid to valid-near-native multiplies AutoDock\textquotesingle s unit cost by about 17.3, compared with 4.3 for DiffDock and 6.4 for EquiBind. All three multipliers are taken on the unrounded medians. The multiplier is large for AutoDock because a complex that succeeds contributes only one or two qualifying poses out of thirty, not because it succeeds rarely. EquiBind\textquotesingle s 2.4 s median describes only the 83 successful complexes. It fails in the other 220.
...
Removing parallelism from the resource axis widens the separation (Figure~\ref{fig:results-r24} and Table~\ref{tab:results-cost-per-qualifying-pose}). On the reported basis configured AutoDock is most expensive on every axis. Its 32 threads compress 151.42 CPU-core-hours of search into 4.73 wall-hours. The subsequent GPU rescoring pass adds nothing to the CPU column, although it did occupy host cores that this accounting does not count. The tools convert their compute into 442 qualifying poses for AutoDock, 2,068 for DiffDock and 483 for EquiBind, which explains much of the gap. GPU time covers docking for DiffDock and EquiBind but only gnina rescoring for AutoDock. AutoDock nevertheless has the highest GPU cost per qualifying pose, partly because its qualifying yield is low and partly because its GPU column sums sixteen concurrent workers.

Four caveats apply. First, wall-clock time is cross-hardware and device mixes differ within and between pipelines. AutoDock spends 4.73 of its 15.00 charged hours on a multithreaded CPU and 10.27 on a GPU shared by up to sixteen concurrent workers. That second figure counts each worker separately and exceeds the 0.72 hours the device was busy. Second, resource values are order-of-magnitude estimates. AutoDock CPU-core-seconds cover only Vina and assume all 32 threads are occupied, while gnina is recorded separately in the GPU column. Third, part of the AutoDock rescoring pass is an implementation artefact. The rescorer was invoked once per pose, and each invocation paid the start-up of a separate process on a GPU that a single-pose evaluation leaves substantially idle. Batching would therefore reduce the cost of the learned rescorer by an amount this run does not determine. Fourth, the charged basis credits every pipeline with full use of the 32 threads, which the three arms did not realise equally. Vina already occupies all 32 within a single docking, whereas EquiBind reaches that occupancy only by running separate complexes concurrently, so its charged figure is an idealised best-configuration estimate rather than a measured elapsed time. Its 21.39 CPU-core-hours are under a seventh of AutoDock\textquotesingle s 151.42. The uniform basis therefore makes the three columns arithmetically comparable without making the underlying concurrency equally attainable, and EquiBind\textquotesingle s figure should be read as a floor. The pooled resource panel has no inferential test, but the share of complexes with at least one PoseBusters-valid pose differs strongly between pipelines (Cochran\textquotesingle s Q = 61.1, p = 5.5\ensuremath{\times}10\textsuperscript{\ensuremath{-}14}).

Under the blind whole-protein configuration used throughout this chapter, and on the basis reported here, configured AutoDock is most expensive per qualifying pose in charged, CPU and GPU terms. Only the CPU-core result is independent of how the concurrent rescoring pass is charged. This does not apply to the classical search alone. Without gnina, Vina requires a median 17.8 s and no GPU time per qualifying pose, still below DiffDock on the charged basis. Its CPU cost per qualifying pose is 1,264.8 s, higher than the rescored arm\textquotesingle s 1,233.3 s, because rescoring adds eleven qualifying poses without adding to the CPU column. Vina remains by far the most processor-intensive engine on either arm, while AutoDock becomes most expensive in charged and GPU time only after learned rescoring. It is also not cheapest per generated pose on any currency (Table~\ref{tab:appendix-cost-per-pose}). Its advantage is coverage and admissibility, not economy.

The ordering also depends on the denominator. Median charged time per attempted docking is 157.7 s for AutoDock, 227.6 s for DiffDock and 8.1 s for EquiBind. Because each produces about thirty poses per complex, this rescales the per-complex median rather than providing an independent measure. Allocating the whole campaign to complexes that yield a qualifying pose gives 252, 443 and 37 s. On this denominator AutoDock is cheaper than DiffDock in charged and GPU terms, with 173 versus 442 GPU-seconds per success, but remains much more expensive in CPU-core-seconds. Neither denominator changes the per-qualifying-pose result on the reported basis, although charging the rescoring pass at its measured occupancy does.
...
Within the whole-protein configuration, EquiBind had the shortest median charged cost per qualifying pose, followed by DiffDock and AutoDock. All three are charged by one formula on a 32-thread processor and a single GPU, but the formula is not equally faithful across the arms, since the GPU term sums concurrent workers on the AutoDock arm and, far more weakly, on the EquiBind one. What also differs is how attainable each pipeline\textquotesingle{}s concurrency is, since EquiBind reaches full thread occupancy only across separate complexes, and how many complexes each ordering rests on, EquiBind succeeding on 83 of 303 against 183 for DiffDock. Both the ordering and its magnitude are therefore conditional, the first on how the concurrent rescoring pass is charged and the second on how few complexes EquiBind solves. Within this protocol and on the basis reported, configured AutoDock was the most expensive route on the charged and hardware axes. Charging its rescoring pass at the measured device occupancy leaves only the processor axis unchanged. Its return was coverage and validity rather than speed.
...
Across 303 whole-protein searches, median charged cost per qualifying pose was 2.4 s for EquiBind, 39.0 s for DiffDock and 91.8 s for AutoDock. DiffDock succeeded on 183 complexes, AutoDock on 214 and EquiBind on 83. All three are charged by one formula, CPU-core-seconds divided by the 32 hardware threads of the workstation plus GPU-seconds. The formula treats the arms alike but its GPU term does not, because the AutoDock rescoring pass ran sixteen ways on one device and is charged for every worker. Every tool is charged for the configuration carried forward and for nothing else, so the selection work each pipeline discarded on the way to that configuration is excluded on all three arms alike. The sixteenfold EquiBind advantage over DiffDock is untouched by that asymmetry, because neither arm ran a GPU stage concurrently at scale, and is a property of the work performed rather than of the accounting, though it is conditional on EquiBind succeeding on a quarter of the set and on a thread occupancy it reaches only across separate complexes.

On the basis reported here, configured AutoDock was most expensive per qualifying pose in charged, CPU-core and GPU seconds. Only the CPU-core result is independent of that choice, and on it AutoDock leads by an order of magnitude. Charging the rescoring pass at the 0.72 hours the GPU was busy rather than the 10.27 summed across its sixteen workers puts AutoDock below DiffDock in GPU terms, and below it on the reported medians in charged terms. On the 54 complexes all three tools solve the two are then no longer separable. The claim of greatest expense is conditional on the accounting rather than a property of the method. This does not apply to Vina alone either. Removing gnina puts its charged and GPU cost below DiffDock, although Vina retains the highest CPU-core cost. The 4.73-hour blind search and 10.27-hour gnina pass total 15.00 charged hours, 68.5\% of which belongs to rescoring, against 5.45 hours and 13.2\% on the measured occupancy. Per-pose process start-up leaves the GPU underused, so batching should reduce this cost by an unmeasured amount. These results describe the implementations and hardware, not method classes.
