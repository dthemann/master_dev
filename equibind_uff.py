...
_gpu_lock = threading.Lock()           # Serialize GPU inference (model forward pass)
_rec_graph_lock = threading.Lock()     # Protect receptor graph cache writes
...
def _dock_single_ligand_inproc(
    ...
    # ── GPU inference (serialized across threads) ──
    with _gpu_lock:
...
def _run_equibind(protein_pdb: Path, ligand_file: Path, output_dir: Path,
    ...
    # ── Get or cache receptor graph (thread-safe) ──
    protein_key = str(protein_pdb)
    if protein_key not in _rec_graph_cache:
        with _rec_graph_lock:
            if protein_key not in _rec_graph_cache:  # double-check after lock
...
def _generate_conformers_for_seed(mol_block: str, seed: int, per_seed: int,
    ...
    """Generate conformers for a single seed. Thread-safe (uses mol_block copy)."""
...
def _generate_conformer_sdfs(ligand_file: Path, output_dir: Path,
    ...
    When PARALLEL_CONFORMER_GEN is True, seeds are processed concurrently
    using a thread pool (RDKit C++ releases the GIL).
    """
    if seeds is None:
        seeds = RDKIT_SEEDS
    if per_seed is None:
    ...
    # Serialize mol to MolBlock for thread-safe copies
    ...
    if PARALLEL_CONFORMER_GEN and len(seeds) > 1:
        ...
        with ThreadPoolExecutor(max_workers=n_workers) as executor:
            futures = {
                executor.submit(_generate_conformers_for_seed, mol_block, seed, per_seed, output_dir): seed
                for seed in seeds
            }
            for future in as_completed(futures):
                try:
                    ...
                    monitor.warning(f"Conformer generation failed for seed {futures[future]}: {e}")
    else:
        # Sequential fallback
        for seed in seeds:
            conformer_files.extend(
                _generate_conformers_for_seed(mol_block, seed, per_seed, output_dir)
            )

    monitor.info(f"RDKit generated {len(conformer_files)} conformers for {ligand_file.name}"
                 f" ({'parallel' if PARALLEL_CONFORMER_GEN else 'sequential'})")
    return conformer_files
...
def dock_guided_by_pockets(
    ...
    # ── A+B. Guided by fpocket and p2rank pockets (parallel or sequential) ──
    all_pockets = (
        [(p, "fpocket") for p in fpocket_pockets]
        + [(p, "p2rank") for p in p2rank_pockets]
    )

    if PARALLEL_POCKETS and len(all_pockets) > 1:
        monitor.section(
            f"Pocket-guided docking  ({len(fpocket_pockets)} fpocket + {len(p2rank_pockets)} p2rank "
            f"× {poses_per_pocket} poses = {len(all_pockets) * poses_per_pocket} total, PARALLEL)"
        )
        n_workers = min(N_PARALLEL_WORKERS, len(all_pockets))
        with ThreadPoolExecutor(max_workers=n_workers) as executor:
            futures = {}
            for pocket, source in all_pockets:
                monitor.info(
                    f"[submit] {pocket.unique_id}  score={pocket.score:.2f}  "
                    f"center=({pocket.center[0]:.1f}, {pocket.center[1]:.1f}, {pocket.center[2]:.1f})"
                )
                fut = executor.submit(
                    _dock_pocket_multi_pose,
                    pocket=pocket,
                ...
                futures[fut] = (pocket, source)

            for future in as_completed(futures):
                pocket, source = futures[future]
                try:
                    pocket_results = future.result()
                except Exception as e:
                    monitor.warning(f"Pocket {pocket.unique_id} failed with exception: {e}")
                    pocket_results = [GuidedPoseResult(
                        protein_name=protein_name, ligand_name=ligand_name,
                        mode=source, pocket_id=pocket.pocket_id,
                        pocket_unique_id=pocket.unique_id, pose_num=1,
                        pocket_center=pocket.center, pose_centroid=None,
                        sdf_path=None, success=False,
                ...
                monitor.info(f"  {pocket.unique_id}: {ok_count}/{poses_per_pocket} OK, {fail_count} failed")
                if fail_count > 0:
    ...
    # Phase 1: Dock all unguided poses (GPU-bound, sequential)
    _unguided_docked: List[dict] = []  # collect docked info for batch UFF
    unguided_count = 0
    ...
    # Phase 2: UFF minimize all docked unguided poses (parallel when enabled)
    if UFF_MINIMIZE and _unguided_docked:
        def _uff_one_unguided(info):
            """UFF-minimize a single unguided pose. Thread-safe."""
            minimized_sdf = info["pose_dir"] / "output_uff_minimized.sdf"
            uff_ok, e_before, e_after, uff_msg = _uff_minimize_pose(
                docked_sdf=info["out_sdf"],
                protein_pdb=prepared_protein,
                output_sdf=minimized_sdf,
            )
            return {**info, "uff_ok": uff_ok, "e_before": e_before,
                    "e_after": e_after, "uff_msg": uff_msg,
                    "minimized_sdf": minimized_sdf}

        if PARALLEL_UFF and len(_unguided_docked) > 1:
            n_workers = min(N_PARALLEL_WORKERS, len(_unguided_docked))
            monitor.info(f"UFF minimizing {len(_unguided_docked)} unguided poses in parallel "
                         f"({n_workers} workers)")
            with ThreadPoolExecutor(max_workers=n_workers) as executor:
                uff_results = list(executor.map(_uff_one_unguided, _unguided_docked))
        else:
            uff_results = [_uff_one_unguided(info) for info in _unguided_docked]

        for info in uff_results:
            pose_num = info["pose_num"]
            out_sdf = info["out_sdf"]
            final_sdf = info["final_sdf"]
            pose_dir = info["pose_dir"]
            _uff_minimized = False
            _uff_e_before = None
            _uff_e_after = None

            if info["uff_ok"] and info["minimized_sdf"].exists():
                out_sdf = info["minimized_sdf"]
                _uff_minimized = True
                _uff_e_before = info["e_before"]
                _uff_e_after = info["e_after"]
                monitor.info(f"Unguided pose {pose_num}: UFF minimized "
                             f"(E {info['e_before']:.1f}→{info['e_after']:.1f}, {info['uff_msg']})")
            else:
                monitor.warning(f"Unguided pose {pose_num}: UFF minimization failed "
                                f"({info['uff_msg']}), using raw docked pose")
            ...
            try:
                ...
                pass
    elif _unguided_docked:
        # UFF disabled — just copy poses directly
        for info in _unguided_docked:
            out_sdf = info["out_sdf"]
            final_sdf = info["final_sdf"]
            pose_dir = info["pose_dir"]
            pose_num = info["pose_num"]
...
print(f"    Parallel conformer generation:   {'ON' if PARALLEL_CONFORMER_GEN else 'OFF'}")
print(f"    Parallel pocket processing:      {'ON' if PARALLEL_POCKETS else 'OFF'}")
print(f"    Parallel UFF minimization:       {'ON' if PARALLEL_UFF else 'OFF'}")
...
full_summary = {
    ...
    "config": {
        ...
        "parallel_conformer_gen": PARALLEL_CONFORMER_GEN,
        "parallel_pockets": PARALLEL_POCKETS,
        "parallel_uff": PARALLEL_UFF,
    },
    "totals": {
